...
class Items extends MY_Controller
{
	...
	protected function validate_access_to_items()
	{
		...
		{
			if($access_invoice->access_type === "all" || $access_estimate->access_type === "all")
			{
				return true;
			}
			else
	...
	{
		$categories = $this->Item_categories_model->get_all_where(["deleted" => 0], 0, 0, "title")->result();
	...
	function modal_form()
	{
		...
		$view_data['categories_dropdown'] = $this->Item_categories_model->get_dropdown_list(["title"]);
	...
	function save()
	{
		...
		if($item_id)
		{
			...
			echo json_encode(["success" => true, "id" => $item_info->id, "data" => $this->_make_item_row($item_info), 'message' => lang('record_saved')]);
		}
		else
		{
	...
	function delete()
	{
		...
		if($this->input->post('undo'))
		{
			if($this->Items_model->delete($id, true))
			{
				$options = ["id" => $id];
				$item_info = $this->Items_model->get_details($options)->row();
				echo json_encode(["success" => true, "id" => $item_info->id, "data" => $this->_make_item_row($item_info), "message" => lang('record_undone')]);
			}
			else
			{
		...
		else
		{
			...
			{
				$item_info = $this->Items_model->get_one($id);
				echo json_encode(["success" => true, "id" => $item_info->id, 'message' => lang('record_deleted')]);
			}
			else
			{
	...
	private function _make_item_row($data)
	{
		...
		return [
			modal_anchor(get_uri("items/view"), $show_in_client_portal_icon . $data->title, ["title" => lang("item_details"), "data-post-id" => $data->id]),
			nl2br($data->description),
			$data->category_title ? $data->category_title : "-",
			$type,
			$data->rate,
			modal_anchor(get_uri("items/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_item'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("items/delete"), "data-action" => "delete"]),
		];
	}
	...
	function validate_items_file()
	{
		...
		{
			echo json_encode(["success" => false, 'message' => lang('please_upload_valid_image_files')]);
		}
	}
	...
	function view()
	{
		...
		$model_info = $this->Items_model->get_details(["id" => $this->input->post('id'), "login_user_id" => $this->login_user->id])->row();
	...
	function grid_view($offset = 0, $limit = 20, $category_id = 0, $search = "")
	{
		...
		else
		{
			...
			{ //search suggestions view
				echo json_encode(["success" => true, "data" => $this->load->view("items/items_grid_data", $view_data, true)]);
			}
			else
			{ //default view
	...
	function count_cart_items()
	{
		...
		$cart_items_count = $this->Order_items_model->get_all_where(["created_by" => $this->login_user->id, "order_id" => 0, "deleted" => 0])->num_rows();

		if($cart_items_count)
		{
			echo json_encode(["success" => true, "cart_items_count" => $cart_items_count]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('no_record_found')]);
		}
	}
	...
	function delete_cart_item()
	{
		...
		{
			echo json_encode(["success" => true, 'message' => lang('record_deleted'), "cart_total_view" => $this->_get_cart_total_view()]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('record_cannot_be_deleted')]);
	...
	function change_cart_item_quantity()
	{
		...
		if($item_info->id)
		{
			...
			echo json_encode(["success" => true, 'message' => lang('record_saved'), "data" => $this->load->view("items/cart/cart_item_data", $view_data, true), "cart_total_view" => $this->_get_cart_total_view()]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
