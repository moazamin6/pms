...
class Dashboard extends MY_Controller
{
	...
	function save_custom_widget()
	{
		...
		if($save_id)
		{
			...
			echo json_encode(["success" => true, "id" => $save_id, "custom_widgets_row" => $this->_make_widgets_row($custom_widgets_data), "custom_widgets_data" => $this->_widgets_row_data($custom_widgets_data), 'message' => lang('record_saved')]);
		}
		else
		{
	...
	{
		$view_data["dashboards"] = $this->Dashboards_model->get_details(["user_id" => $this->login_user->id])->result();
	...
	private function _get_my_dashboard($id = 0)
	{
		if($id)
		{
			return $this->Dashboards_model->get_details(["user_id" => $this->login_user->id, "id" => $id])->row();
		}
	}
	...
	private function _get_my_custom_widget($id = 0)
	{
		if($id)
		{
			return $this->Custom_widgets_model->get_details(["user_id" => $this->login_user->id, "id" => $id])->row();
		}
	}
	...
	function save()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "dashboard_id" => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function delete_custom_widgets()
	{
		...
		{
			echo json_encode(["success" => true, "id" => $id, 'message' => lang('record_deleted')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('record_cannot_be_deleted')]);
	...
	private function _make_widgets($dashboard_id = 0)
	{
		...
		//custom widgets
		$custom_widgets = $this->Custom_widgets_model->get_details(["user_id" => $this->login_user->id])->result();
		if($custom_widgets)
		{
	...
	private function _widgets_row_data($widget_array)
	{
		...
		if(is_numeric($key))
		{
			...
			$widgets_title = $value;
			$details_button = modal_anchor(get_uri("dashboard/view_custom_widget"), "<i class='fa fa-ellipsis-h'></i>", ["class" => "text-off pr10 pl10", "title" => lang('custom_widget_details'), "data-post-id" => $key]);
		}
		else
		{
			$details_button = modal_anchor(get_uri("dashboard/view_default_widget"), "<i class='fa fa-ellipsis-h'></i>", ["class" => "text-off pr10 pl10", "title" => lang($key), "data-post-widget" => $key]);
										...
										else
										{
											...
											else
											{
												...
												else
												{
													...
													{
														return $this->load->view("dashboards/custom_dashboards/extra_data/widget_with_heading", ["icon" => "fa-clock-o", "widget" => $widget], true);
													}
													else
													{
														if($widget == "task_status")
														{
															return my_task_stataus_widget("h370", true);
														}
														else
														{
															if($widget == "sticky_note")
															{
																return sticky_note_widget("h370", true);
															}
															else
															{
																if($widget == "all_tasks_kanban")
																{
																...
																else
																{
																	...
																	{
																		if($widget == "open_projects")
																		{
																			return open_projects_widget("", true);
																		}
																		else
																		{
																			if($widget == "completed_projects")
																			{
																			...
																			else
																			{
																				...
																				{
																					if($widget == "members_clocked_out")
																					{
																						return count_clock_out_widget(true);
																					}
																					else
																					{
																						if($widget == "open_projects_list")
																						{
																							return my_open_projects_widget("", true);
																						}
																						else
																						{
																							if($widget == "starred_projects")
																							{
																								return my_starred_projects_widget("", true);
																							}
																							else
																							{
																								if($widget == "new_tickets" || $widget == "open_tickets" || $widget == "closed_tickets")
																								{
																									$this->init_permission_checker("ticket");
																									$explode_widget = explode("_", $widget);
																									return ticket_status_widget_small(["status" => $explode_widget[0], "allowed_ticket_types" => $this->allowed_ticket_types, "show_assigned_tickets_only_user_id" => $this->show_assigned_tickets_only_user_id()], true);
																								}
																								else
																								{
																									if($widget == "all_team_members")
																									{
																										return all_team_members_widget(true);
																									}
																									else
																									{
																										if($widget == "clocked_in_team_members")
																										{
																											$this->init_permission_checker("attendance");
																											return clocked_in_team_members_widget(["access_type" => $this->access_type, "allowed_members" => $this->allowed_members], true);
																										}
																										else
																										{
																											if($widget == "clocked_out_team_members")
																											{
																												$this->init_permission_checker("attendance");
																												return clocked_out_team_members_widget(["access_type" => $this->access_type, "allowed_members" => $this->allowed_members], true);
																											}
																											else
																											{
																												if($widget == "latest_online_team_members")
																												{
																													return active_members_and_clients_widget("staff", true);
																												}
																												else
																												{
																													if($widget == "total_invoices" || $widget == "total_payments" || $widget == "total_due" || $widget == "draft_invoices_value")
																													{
																														$explode_widget = explode("_", $widget);
																													...
																													{
																														if($widget == "my_tasks_list")
																														{
																															return my_tasks_list_widget(true);
																														}
																														else
																														{
																															if($widget == "all_timesheets_statistics")
																															{
																																return project_timesheet_statistics_widget("all_timesheets_statistics", true);
																															}
																															else
																															{
																																if($widget == "pending_leave_approval")
																																{
																																	$this->init_permission_checker("leave");
																																	return pending_leave_approval_widget(true, ["access_type" => $this->access_type, "allowed_members" => $this->allowed_members]);
																																}
																																else
																																{
																																	if($widget == "total_hours_worked" || $widget == "total_project_hours")
																																	{
																																		return count_total_time_widget_small(0, $widget, true);
																																	}
																																	else
																																	{
																																		if($widget == "active_members_on_projects")
																																		{
																																			return active_members_on_projects_widget(true);
																																		}
																																		else
																																		{
																																			if($widget == "draft_invoices")
																																			{
																																				return draft_invoices_widget(true);
																																			}
																																			else
																																			{
																																				if($widget == "total_clients" || $widget == "total_contacts" || $widget == "latest_online_client_contacts")
																																				{
																																					...
																																					{
																																						if($widget == "total_contacts")
																																						{
																																							return total_contacts_widget(true, $show_own_clients_only_user_id);
																																						}
																																						else
																																						{
																																							if($widget == "latest_online_client_contacts")
																																							{
																																				...
																																				{
																																					if($widget == "open_tickets_list")
																																					{
																																						return open_tickets_list_widget(true);
																																					}
																																				}
																																			}
																																		}
	...
	private function _get_widgets_for_client($widget, $widgets_array)
	{
		...
		if(get_array_value($widgets_array, $widget))
		{
			...
			{
				return $this->load->view("clients/info_widgets/tab", ["tab" => "projects", "client_info" => $client_info], true);
			}
			else
			{
				if($widget == "total_invoices")
				{
					return $this->load->view("clients/info_widgets/tab", ["tab" => "invoice_value", "client_info" => $client_info], true);
				}
				else
				{
					if($widget == "total_payments")
					{
						return $this->load->view("clients/info_widgets/tab", ["tab" => "payments", "client_info" => $client_info], true);
					}
					else
					{
						if($widget == "total_due")
						{
							return $this->load->view("clients/info_widgets/tab", ["tab" => "due", "client_info" => $client_info], true);
						}
						else
						{
							if($widget == "open_projects_list")
										...
										else
										{
											...
											else
											{
												...
												else
												{
													...
													{
														$explode_widget = explode("_", $widget);
														return ticket_status_widget_small(["status" => $explode_widget[0]], true);
													}
													else
													{
														if($widget == "project_timeline")
