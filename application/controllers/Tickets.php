...
class Tickets extends MY_Controller
{
	...
	function modal_form()
	{
		...
		{
			$where = ["where_in" => ["id" => $this->allowed_ticket_types]];
		}
		
		$ticket_info = $this->Tickets_model->get_one($this->input->post("id"));
		
		$projects = $this->Projects_model->get_dropdown_list(["title"], "id", ["client_id" => $ticket_info->client_id]);
		if($this->login_user->user_type == "client")
		{
			$projects = $this->Projects_model->get_dropdown_list(["title"], "id", ["client_id" => $this->login_user->client_id]);
		...
		$view_data['ticket_types_dropdown'] = $this->Ticket_types_model->get_dropdown_list(["title"], "id", $where);
		...
		{
			$view_data['projects_dropdown'] = $this->Projects_model->get_dropdown_list(["title"]);
		}
		
		$requested_by_suggestion = [["id" => "", "text" => "-"]];
	...
	function get_project_suggestion($client_id = 0)
	{
		...
		$projects = $this->Projects_model->get_dropdown_list(["title"], "id", ["client_id" => $client_id]);
		$suggestion = [["id" => "", "text" => "-"]];
		foreach($projects as $key => $value)
		{
			$suggestion[] = ["id" => $key, "text" => $value];
	...
	function save()
	{
		...
		if($ticket_id)
		{
			...
			if(!$id)
			{
				...
				{
					log_notification("ticket_created", ["ticket_id" => $ticket_id, "ticket_comment_id" => $ticket_comment_id]);
				}
			...
			else
			{
				if($assigned_to)
				{
					log_notification("ticket_assigned", ["ticket_id" => $ticket_id, "to_user_id" => $assigned_to]);
				}
			}
			
			echo json_encode(["success" => true, "data" => $this->_row_data($ticket_id), 'id' => $ticket_id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	private function _make_row($data, $custom_fields)
	{
		...
		if($this->login_user->user_type == "staff")
		{
			...
			if($ticket_info)
			{
				...
				$edit = '<li role="presentation">' . modal_anchor(get_uri("tickets/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), ["title" => lang('edit'), "data-post-view" => "details", "data-post-id" => $ticket_info->id]) . '</li>';
				
				//show option to close/open the tickets
				$status = "";
				if($ticket_info->status === "closed")
				{
					$status = '<li role="presentation">' . js_anchor("<i class='fa fa-check-circle'></i> " . lang('mark_as_open'), ['title' => lang('mark_as_open'), "class" => "", "data-action-url" => get_uri("tickets/save_ticket_status/$ticket_info->id/open"), "data-action" => "update"]) . '</li>';
				}
				else
				{
					$status = '<li role="presentation">' . js_anchor("<i class='fa fa-check-circle'></i> " . lang('mark_as_closed'), ['title' => lang('mark_as_closed'), "class" => "", "data-action-url" => get_uri("tickets/save_ticket_status/$ticket_info->id/closed"), "data-action" => "update"]) . '</li>';
				}
				
				$assigned_to = "";
				if($ticket_info->assigned_to === "0")
				{
					$assigned_to = '<li role="presentation">' . js_anchor("<i class='fa fa-user'></i> " . lang('assign_to_me'), ['title' => lang('assign_myself_in_this_ticket'), "data-action-url" => get_uri("tickets/assign_to_me/$ticket_info->id"), "data-action" => "update"]) . '</li>';
				}
				...
				{
					$delete_ticket = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), ['title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("tickets/delete"), "data-action" => "delete-confirmation"]) . '</li>';
				}
	...
	function save_comment()
	{
		...
		if($comment_id)
		{
			...
			$comments_options = ["id" => $comment_id];
			...
			echo json_encode(["success" => true, "data" => $comment_view, 'message' => lang('comment_submited')]);
			
			log_notification("ticket_commented", ["ticket_id" => $ticket_id, "ticket_comment_id" => $comment_id]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function save_ticket_status($ticket_id = 0, $status = "closed")
	{
		if($ticket_id)
		{
			...
			if($save_id)
			{
				if($status == "open")
				{
					log_notification("ticket_reopened", ["ticket_id" => $ticket_id]);
				}
				else
				{
					if($status == "closed")
					{
						log_notification("ticket_closed", ["ticket_id" => $ticket_id]);
						
						//save closing time
						$closed_data = ["closed_at" => get_current_utc_time()];
						$this->Tickets_model->save($closed_data, $ticket_id);
					}
				}
				
				echo json_encode(["success" => true, "data" => $this->_row_data($ticket_id), "id" => $ticket_id, "message" => ($status == "closed") ? lang('ticket_closed') : lang('ticket_reopened')]);
			}
			else
			{
	...
	function save_ticket_template()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, 'id' => $save_id, "data" => $this->_row_data_for_ticket_templates($save_id), 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	{
		$options = ["created_by" => $this->login_user->id, "ticket_type_id" => $ticket_type_id];
		if($this->login_user->user_type === "staff" && $this->access_type !== "all" && $this->access_type !== "assigned_only")
		{
	...
	private function _make_row_for_ticket_templates($data, $view_type = "")
	{
		...
		{
			$title = modal_anchor(get_uri("tickets/ticket_template_view/" . $data->id), $data->title, ["class" => "edit", "title" => lang('ticket_template'), "data-post-id" => $data->id]);
		...
		$actions = modal_anchor(get_uri("tickets/ticket_template_view/" . $data->id), "<i class='fa fa-bolt'></i>", ["class" => "edit", "title" => lang('template_details'), "data-modal-title" => lang('template'), "data-post-id" => $data->id]);
		if($data->created_by == $this->login_user->id || $this->login_user->is_admin)
		{
			$actions = modal_anchor(get_uri("tickets/ticket_template_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_template'), "data-post-id" => $data->id])
				. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("tickets/delete_ticket_template"), "data-action" => "delete-confirmation"]);
		}
	...
	function add_client_modal_form($ticket_id = 0)
	{
		if($ticket_id)
		{
			...
			$view_data['clients_dropdown'] = ["" => "-"] + $this->Clients_model->get_dropdown_list(["company_name"]);
	...
	function get_client_contact_suggestion($client_id = 0)
	{
		...
		$clients = $this->Users_model->get_dropdown_list(["first_name", "last_name"], "id", ["deleted" => 0, "client_id" => $client_id]);
		$suggestion = [["id" => "", "text" => "-"]];
	...
	private function _get_ticket_types_dropdown_list_for_filter()
	{
		...
		$ticket_type = $this->Ticket_types_model->get_dropdown_list(["title"], "id", $where);
		
		$ticket_type_dropdown = [["id" => "", "text" => "- " . lang("ticket_type") . " -"]];
		foreach($ticket_type as $id => $name)
		{
			$ticket_type_dropdown[] = ["id" => $id, "text" => $name];
		}
