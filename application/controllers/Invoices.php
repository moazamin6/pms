...
class Invoices extends MY_Controller
{
	...
	function modal_form()
	{
		...
		$projects = $this->Projects_model->get_dropdown_list(["title"], "id", ["client_id" => $project_client_id]);
		$suggestion = [["id" => "", "text" => "-"]];
	...
	function save()
	{
		...
		if($recurring)
		{
			...
			{
				echo json_encode(["success" => false, 'message' => lang('past_recurring_date_error_message_title'), 'next_recurring_date_error' => lang('past_recurring_date_error_message'), "next_recurring_date_value" => $invoice_data['next_recurring_date']]);
				return false;
			}
		}
		...
		if($invoice_id)
		{
			...
			if($is_clone && $main_invoice_id)
			{
				...
				$invoice_items = $this->Invoice_items_model->get_all_where(["invoice_id" => $main_invoice_id, "deleted" => 0])->result();
			...
			echo json_encode(["success" => true, "data" => $this->_row_data($invoice_id), 'id' => $invoice_id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	private function _copy_estimate_or_order_items_to_invoice($copy_items_from_estimate, $copy_items_from_order, $invoice_id)
	{
		...
		{
			$items = $this->Estimate_items_model->get_details(["estimate_id" => $copy_items_from_estimate])->result();
		}
		else
		{
			if($copy_items_from_order)
			{
				$items = $this->Order_items_model->get_details(["order_id" => $copy_items_from_order])->result();
			}
		}
	...
	{
		$edit = '<li role="presentation">' . modal_anchor(get_uri("invoices/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), ["title" => lang('edit_invoice'), "data-post-id" => $invoice_id]) . '</li>';
		
		$delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), ['title' => lang('delete_invoice'), "class" => "delete", "data-id" => $invoice_id, "data-action-url" => get_uri("invoices/delete"), "data-action" => "delete-confirmation"]) . '</li>';
		
		$add_payment = '<li role="presentation">' . modal_anchor(get_uri("invoice_payments/payment_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_payment'), ["title" => lang('add_payment'), "data-post-invoice_id" => $invoice_id]) . '</li>';
	...
	function save_item()
	{
		...
		if($invoice_item_id)
		{
			...
			echo json_encode(["success" => true, "invoice_id" => $item_info->invoice_id, "data" => $this->_make_item_row($item_info), "invoice_total_view" => $this->_get_invoice_total_view($item_info->invoice_id), 'id' => $invoice_item_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function delete_item()
	{
		...
		if($this->input->post('undo'))
		{
			if($this->Invoice_items_model->delete($id, true))
			{
				$options = ["id" => $id];
				$item_info = $this->Invoice_items_model->get_details($options)->row();
				echo json_encode(["success" => true, "invoice_id" => $item_info->invoice_id, "data" => $this->_make_item_row($item_info), "invoice_total_view" => $this->_get_invoice_total_view($item_info->invoice_id), "message" => lang('record_undone')]);
			}
			else
			{
	...
	function item_list_data($invoice_id = 0)
	{
		...
		$list_data = $this->Invoice_items_model->get_details(["invoice_id" => $invoice_id])->result();
		$result = [];
		foreach($list_data as $data)
		{
	...
	private function _make_item_row($data)
	{
		...
		return [
			...
			modal_anchor(get_uri("invoices/item_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_invoice'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("invoices/delete_item"), "data-action" => "delete"]),
		];
	}
	...
	function print_invoice($invoice_id = 0)
	{
		if($invoice_id)
		{
			...
			echo json_encode(["success" => true, "print_view" => $this->load->view("invoices/print_invoice", $view_data, true)]);
		}
		else
		{
			echo json_encode(["success" => false, lang('error_occurred')]);
	...
	function send_invoice()
	{
		...
		{
			// change email status
			$status_data = ["status" => "not_paid", "last_email_sent_date" => get_my_local_time()];
			if($this->Invoices_model->save($status_data, $invoice_id))
			{
				echo json_encode(['success' => true, 'message' => lang("invoice_sent_message"), "invoice_id" => $invoice_id]);
			}
	...
	function get_invoice_status_bar($invoice_id = 0)
	{
		...
		$view_data["invoice_info"] = $this->Invoices_model->get_details(["id" => $invoice_id])->row();
	...
	function save_discount()
	{
		...
		if($save_data)
		{
			echo json_encode(["success" => true, "invoice_total_view" => $this->_get_invoice_total_view($invoice_id), 'message' => lang('record_saved'), "invoice_id" => $invoice_id]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function load_statistics_of_selected_currency($currency = "")
	{
		if($currency)
		{
			$statistics = invoice_statistics_widget(true, ["currency" => $currency]);
