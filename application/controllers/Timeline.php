...
class Timeline extends MY_Controller
{
	...
	function save()
	{
		...
		if($save_id)
		{
			...
			echo json_encode(["success" => true, "data" => $data, 'message' => lang('comment_submited')]);
			
			if($post_id == 0)
			{
				log_notification("created_a_new_post", ["post_id" => $save_id]);
			}
			else
			{
				log_notification("timeline_post_commented", ["post_id" => $save_id]);
			}
		}
		else
		{
	...
	{
		$view_data['reply_list'] = $this->Posts_model->get_details(["post_id" => $post_id])->result;
	...
	function post($post_id)
	{
		...
		$post = $this->Posts_model->get_details(["id" => $post_id]);
