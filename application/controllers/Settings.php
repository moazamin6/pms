...
class Settings extends MY_Controller
{
	...
	function save_general_settings()
	{
		...
		foreach($settings as $setting)
		{
			...
			if($value || $value === "0")
			{
				...
				{
					if($setting === "item_purchase_code" && $value === "******")
					{
		...
		if($_FILES)
		{
			$files = ["site_logo_file", "favicon_file"];
		...
		echo json_encode(["success" => true, 'message' => lang('settings_updated'), 'reload_page' => $reload_page]);
	}
	...
	{
		$settings = ["company_name", "company_address", "company_phone", "company_email", "company_website", "company_vat_number"];
	...
	{
		$settings = ["email_sent_from_address", "email_sent_from_name", "email_protocol", "email_smtp_host", "email_smtp_port", "email_smtp_user", "email_smtp_pass", "email_smtp_security_type"];
		...
		if($test_email_to)
		{
			...
			if($this->email->send())
			{
				echo json_encode(["success" => true, 'message' => lang('test_mail_sent')]);
				return false;
			}
			else
			{
				log_message('error', $this->email->print_debugger());
				echo json_encode(["success" => false, 'message' => lang('test_mail_send_failed')]);
				return false;
			}
		}
		echo json_encode(["success" => true, 'message' => lang('settings_updated')]);
	...
	{
		$settings = ["allow_partial_invoice_payment_from_clients", "invoice_color", "invoice_footer", "send_bcc_to", "invoice_prefix", "invoice_style", "invoice_logo", "send_invoice_due_pre_reminder", "send_invoice_due_after_reminder", "send_recurring_invoice_reminder_before_creation", "default_due_date_after_billing_date", "initial_number_of_the_invoice", "client_can_pay_invoice_without_login"];
		$reload_page = false;
	...
	function notification_modal_form()
	{
		...
		if($id)
		{
			...
			$model_info = $this->Notification_settings_model->get_details(["id" => $id])->row();
			...
			{
				$teams = $this->Team_model->get_all_where(["deleted" => 0])->result();
	...
	function save_notification_settings()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_notification_list_data($save_id), 'id' => $save_id, 'message' => lang('settings_updated')]);
		}
		else
		{
	...
	function save_bitbucket_settings()
	{
		...
		$settings = ["enable_bitbucket_commit_logs_in_tasks"];
	...
	function save_ticket_settings()
	{
		...
		$settings = ["show_recent_ticket_comments_at_the_top", "ticket_prefix", "project_reference_in_tickets", "auto_close_ticket_after", "auto_reply_to_tickets", "auto_reply_to_tickets_message", "enable_embedded_form_to_get_tickets"];
	...
	function save_task_settings()
	{
		...
		$settings = ["project_task_reminder_on_the_day_of_deadline", "project_task_deadline_pre_reminder", "project_task_deadline_overdue_reminder", "enable_recurring_option_for_tasks", "task_point_range", "create_recurring_tasks_before"];
	...
	{
		$settings = ["enable_email_piping", "create_tickets_only_by_registered_emails", "imap_ssl_enabled", "imap_host", "imap_port", "imap_email", "imap_password"];
	...
	{
		$settings = ["enable_push_notification", "pusher_app_id", "pusher_key", "pusher_secret", "pusher_cluster", "enable_chat_via_pusher"];
	...
	{
		$settings = ["enable_google_drive_api_to_upload_file", "google_drive_client_id", "google_drive_client_secret"];
	...
	{
		$settings = ["estimate_logo", "estimate_prefix", "estimate_color", "estimate_footer", "send_estimate_bcc_to", "initial_number_of_the_estimate", "create_new_projects_automatically_when_estimates_gets_accepted"];
		$reload_page = false;
	...
	function test_push_notification()
	{
		...
		{
			echo json_encode(["success" => false, 'message' => lang('push_notification_error_message')]);
		}
	}
	...
	{
		$settings = ["enable_gdpr", "allow_clients_to_export_their_data", "clients_can_request_account_removal", "show_terms_and_conditions_in_client_signup_page", "gdpr_terms_and_conditions_link"];
	...
	{
		$edit = modal_anchor(get_uri("settings/footer_item_edit_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "pull-right mr5 footer-menu-edit-btn", "title" => lang('edit_footer_menu'), "data-post-menu_name" => $menu_name, "data-post-url" => $url]);
	...
	{
		$settings = ["enable_footer", "footer_menus", "footer_copyright_text"];
	...
	function save_footer_menu()
	{
		...
		if($menu_name && $url)
		{
			echo json_encode(["success" => true, 'data' => $this->_make_footer_menu_item_data($menu_name, $url, $type)]);
		}
	}
	...
	function estimate_request_settings()
	{
		...
		$hidden_fields_dropdown = [];
		foreach($hidden_fields as $hidden_field)
		{
			$hidden_fields_dropdown[] = ["id" => $hidden_field, "text" => lang($hidden_field)];
		}
	...
	function save_estimate_request_settings()
	{
		...
		foreach($settings as $setting)
		{
			...
			{
				echo json_encode(["success" => false, 'message' => lang("estimate_request_name_email_error_message")]);
				return false;
			}
	...
	{
		$settings = ["order_logo", "order_prefix", "order_color", "order_footer", "initial_number_of_the_order", "order_tax_id", "order_tax_id2"];
		$reload_page = false;
	...
	function projects()
	{
		...
		$project_tabs_dropdown = [];
		foreach($project_tabs as $project_tab)
		{
			$project_tabs_dropdown[] = ["id" => $project_tab, "text" => lang($project_tab)];
		}
	...
	{
		$settings = ["enable_slack", "slack_webhook_url", "slack_dont_send_any_projects"];
	...
	function save_github_settings()
	{
		...
		$settings = ["enable_github_commit_logs_in_tasks"];
