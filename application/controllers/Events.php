...
class Events extends MY_Controller
{
	...
	function save()
	{
		...
		{
			if($share_with == "specific_client_contacts")
			{
		...
		if($save_id)
		{
			...
			if($share_with)
			{
				if($id)
				{
					...
					log_notification("calendar_event_modified", ["event_id" => $save_id]);
				}
				else
				{
					//new event added and shared with others, log the notificaiton
					log_notification("new_event_added_in_calendar", ["event_id" => $save_id]);
				}
			}
		}
		else
	...
	function delete()
	{
		...
		if($this->Events_model->delete($id))
		{
			...
			echo json_encode(["success" => true, 'message' => lang('event_deleted')]);
		}
		else
		{
	...
	function calendar_events($filter_values = "", $event_label_id = 0, $client_id = 0)
	{
		...
		if(in_array("events", $filter_values_array))
		{
			...
			$options_of_events = ["user_id" => $this->login_user->id, "team_ids" => $this->login_user->team_ids, "client_id" => $client_id, "start_date" => $start, "end_date" => $end, "include_recurring" => true, "is_client" => $is_client, "label_id" => $event_label_id];
		...
		if(in_array("leave", $filter_values_array) && $this->login_user->user_type == "staff")
		{
			...
			$options_of_leaves = ["start_date" => $start, "end_date" => $end, "login_user_id" => $this->login_user->id, "access_type" => $leave_access_info->access_type, "allowed_members" => $leave_access_info->allowed_members, "status" => "approved"];
	...
	private function _make_project_event($data, $start_date_event = false)
	{
		...
		if(($data->deadline && ($my_local_time > $data->deadline)) || (!$data->deadline && $data->start_date && ($my_local_time > $data->start_date)))
		{ //back-dated events
			$color = "#d9534f";
		}
		else
	...
	private function _make_view_data($encrypted_event_id, $cycle = "0")
	{
		...
		$model_info = $this->Events_model->get_details(["id" => $event_id])->row();
		...
		if($event_id && $model_info->id)
		{
			...
			$status_confirm = modal_anchor(get_uri("events/save_event_status/"), "<i class='fa fa-check-circle-o'></i> " . lang('confirm'), ["class" => "btn btn-success pull-left", "data-post-encrypted_event_id" => $encrypted_event_id, "title" => lang('event_details'), "data-post-status" => "confirmed", "data-post-editable" => "1"]);
			$status_reject = modal_anchor(get_uri("events/save_event_status/"), "<i class='fa fa-times-circle-o'></i> " . lang('reject'), ["class" => "btn btn-danger pull-left", "data-post-encrypted_event_id" => $encrypted_event_id, "title" => lang('event_details'), "data-post-status" => "rejected", "data-post-editable" => "1"]);
			...
			{
				if(in_array($this->login_user->id, $rejected_by_array))
				{
	...
	private function _get_confirmed_and_rejected_users_list($confirmed_by_array, $rejected_by_array)
	{
		...
		return ["confirmed_by" => $confirmed_by, "rejected_by" => $rejected_by];
	}
	...
	function get_all_contacts_of_client($client_id)
	{
		...
		{
			$client_contacts = $this->Users_model->get_all_where(["status" => "active", "client_id" => $client_id, "deleted" => 0])->result();
			...
			if($client_contacts)
			{
				...
				{
					$client_contacts_array[] = ["type" => "contact", "id" => "contact:" . $contacts->id, "text" => $contacts->first_name . " " . $contacts->last_name];
				}
			}
	...
	function google_calendar_settings_modal_form()
	{
		if(get_setting("enable_google_calendar_api"))
		{
			...
			$calendar_ids = $user_calendar_ids ? unserialize($user_calendar_ids) : [];

			$this->load->view("events/google_calendar_settings_modal_form", ["calendar_ids" => $calendar_ids]);
		}
	}
	...
	function save_google_calendar_settings()
	{
		if(get_setting("enable_google_calendar_api"))
		{
			$settings = ["integrate_with_google_calendar", "google_client_id", "google_client_secret"];
