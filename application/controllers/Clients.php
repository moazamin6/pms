...
class Clients extends MY_Controller
{
	...
	private function _make_file_row($data)
	{
		...
		$description = "<div class='pull-left'>" .
			js_anchor(remove_file_prefix($data->file_name), ['title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("clients/view_file/" . $data->id)]);
		...
		$options = anchor(get_uri("clients/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", ["title" => lang("download")]);
		
		if($this->login_user->user_type == "staff")
		{
			$options .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clients/delete_file"), "data-action" => "delete-confirmation"]);
		}
	...
	{
		//setting preferences
		$settings = ["notification_sound_volume", "disable_push_notification", "disable_keyboard_shortcuts"];
	...
	function contacts_list_data($client_id = 0)
	{
		...
		$options = ["user_type" => "client", "client_id" => $client_id, "custom_fields" => $custom_fields, "show_own_clients_only_user_id" => $this->show_own_clients_only_user_id()];
	...
	private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false)
	{
		...
		$row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("clients/delete_contact"), "data-action" => "delete"]);
	...
	function send_invitation()
	{
		...
		{
			echo json_encode(['success' => true, 'message' => lang("invitation_sent")]);
		}
		else
		{
			echo json_encode(['success' => false, 'message' => lang('error_occurred')]);
	...
	private function _prepare_client_data($data_row, $allowed_headers)
	{
		//prepare client data
		$client_data = [];
		$client_contact_data = ["user_type" => "client", "is_primary_contact" => 1];
	...
	private function _get_client_group_ids($client_groups_data)
	{
		...
		{
			$group_id = "";
			$existing_group = $this->Client_groups_model->get_one_where(["title" => $group, "deleted" => 0]);
	...
	{
		$this->access_only_allowed_members();
		download_app_files(get_setting("system_file_path"), serialize([["file_name" => "import-clients-sample.xlsx"]]));
	}
	...
	{
		$required_general_info_array = ["first_name", "last_name", "email", "job_title", "phone", "gender", "skype", "created_at"];
		...
		$required_client_info_array = ["company_name", "address", "city", "state", "zip", "country", "phone", "website", "vat_number"];
	...
	function request_my_account_removal()
	{
		if(get_setting("enable_gdpr") && get_setting("clients_can_request_account_removal"))
		{
			...
			$data = ["requested_account_removal" => 1];
			$this->Users_model->save($data, $user_id);
			
			$client_id = $this->Users_model->get_one($user_id)->client_id;
			log_notification("client_contact_requested_account_removal", ["client_id" => $client_id], $user_id);
