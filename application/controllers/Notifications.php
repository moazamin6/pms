...
class Notifications extends MY_Controller
{
	...
	function count_notifications()
	{
		...
		echo json_encode(["success" => true, 'total_notifications' => $notifiations]);
	}
	...
	function get_notifications()
	{
		...
		echo json_encode(["success" => true, 'notification_list' => $this->load->view("notifications/list", $view_data, true)]);
	}

	function update_notification_checking_status()
	{
		$now = get_current_utc_time();
		$data = ["notification_checked_at" => $now];
	...
	function set_notification_status_as_read($notification_id = 0)
	{
		...
		else
		{
			...
			echo json_encode(["success" => true, 'message' => lang('marked_all_notifications_as_read')]);
		}
	}
