...
class Left_menus extends MY_Controller
{
	...
	function save()
	{
		...
		if($type == "user")
		{
			...
			echo json_encode(["success" => true, 'redirect_to' => get_uri($this->_prepare_user_custom_redirect_to_url()), 'message' => lang('settings_updated')]);
		}
		else
		{
	...
	function prepare_custom_menu_item_data()
	{
		...
		$item_array = ["name" => $title, "url" => $url, "is_sub_menu" => $is_sub_menu, "icon" => $icon, "open_in_new_tab" => $open_in_new_tab];
		$item_data = $this->left_menu->_get_item_data($item_array);

		if($item_data)
		{
			echo json_encode(["success" => true, "item_data" => $item_data]);
		}
		else
		{
