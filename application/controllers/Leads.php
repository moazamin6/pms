...
class Leads extends MY_Controller
{
	...
	{
		$team_members = $this->Users_model->get_all_where(["user_type" => "staff", "deleted" => 0, "status" => "active"])->result();
		$team_members_dropdown = [];

		if($view_type == "filter")
		{
			$team_members_dropdown = [["id" => "", "text" => "- " . lang("owner") . " -"]];
		}
	...
	function save()
	{
		...
		if($save_id)
		{
			...
			if(!$client_id)
			{
				log_notification("lead_created", ["lead_id" => $save_id], $this->login_user->id);
			}

			echo json_encode(["success" => true, "data" => $this->_row_data($save_id), 'id' => $save_id, 'view' => $this->input->post('view'), 'message' => lang('record_saved')]);
		}
		else
		{
	...
	private function _make_row($data, $custom_fields)
	{
		...
		$row_data[] = js_anchor($data->lead_status_title, ["style" => "background-color: $data->lead_status_color", "class" => "label", "data-id" => $data->id, "data-value" => $data->lead_status_id, "data-act" => "update-lead-status"]);
		...
		$row_data[] = modal_anchor(get_uri("leads/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_lead'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_lead'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("leads/delete"), "data-action" => "delete-confirmation"]);
	...
	private function _make_file_row($data)
	{
		...
		$description = "<div class='pull-left'>" .
			js_anchor(remove_file_prefix($data->file_name), ['title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("leads/view_file/" . $data->id)]);
		...
		$options = anchor(get_uri("leads/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", ["title" => lang("download")]);

		$options .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("leads/delete_file"), "data-action" => "delete-confirmation"]);
	...
	{
		$file_info = $this->General_files_model->get_details(["id" => $file_id])->row();
	...
	function download_file($id)
	{
		...
		//serilize the path
		$file_data = serialize([make_array_of_file($file_info)]);
	...
	function delete_file()
	{
		...
		if($this->General_files_model->delete($id))
		{
			...
			//delete the files
			delete_app_files(get_general_file_path("client", $info->client_id), [make_array_of_file($info)]);
	...
	function save_contact()
	{
		...
		{
			echo json_encode(["success" => false, 'message' => lang('duplicate_email')]);
			exit();
		}
		...
		if($save_id)
		{
			...
			echo json_encode(["success" => true, "data" => $this->_contact_row_data($save_id), 'id' => $contact_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function save_profile_image($user_id = 0)
	{
		...
		if($profile_image)
		{
			...
			//delete old file
			delete_app_files(get_setting("profile_image_path"), [@unserialize($lead_info->image)]);

			$image_data = ["image" => $profile_image];
			$this->Users_model->save($image_data, $user_id);
			echo json_encode(["success" => true, 'message' => lang('profile_image_changed')]);
		}
		...
		if($_FILES)
		{
			...
			if($image_file_name)
			{
				...
				{
					echo json_encode(["success" => false, 'message' => lang('profile_image_error_message')]);
					exit();
				}
				...
				$image_data = ["image" => $profile_image];
				$this->Users_model->save($image_data, $user_id);
				echo json_encode(["success" => true, 'message' => lang('profile_image_changed'), "reload_page" => true]);
			}
		}
	}
	...
	function delete_contact()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_contact_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
	...
	function contacts_list_data($client_id = 0)
	{
		...
		$options = ["user_type" => "lead", "client_id" => $client_id, "custom_fields" => $custom_fields];
	...
	private function _make_contact_row($data, $custom_fields)
	{
		...
		$row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("leads/delete_contact"), "data-action" => "delete"]);
	...
	function make_client_modal_form($lead_id = 0)
	{
		...
		$final_contacts = [];
		$contacts = $this->Users_model->get_all_where(["user_type" => "lead", "deleted" => 0, "status" => "active", "client_id" => $lead_id])->result();
	...
	function save_as_client()
	{
		...
		if($client_id)
		{
			...
			$client_info = $this->Clients_model->get_details(["id" => $client_id])->row();
			...
			{
				echo json_encode(["success" => false, 'message' => lang("account_already_exists_for_your_company_name")]);
				exit();
			}
			...
			if($save_client_id)
			{
				log_notification("client_created_from_lead", ["client_id" => $save_client_id], $this->login_user->id);
				...
				$contacts = $this->Users_model->get_all_where(["user_type" => "lead", "deleted" => 0, "status" => "active", "client_id" => $client_id])->result();
				...
				echo json_encode(["success" => true, 'redirect_to' => get_uri("clients/view/$save_client_id"), "message" => lang('record_saved')]);
			}
			else
			{
				echo json_encode(["success" => false, lang('error_occurred')]);
	...
	private function _prepare_lead_data($data_row, $allowed_headers)
	{
		//prepare lead data
		$lead_data = ["is_lead" => 1];
		$lead_contact_data = ["user_type" => "lead", "is_primary_contact" => 1];
		...
		foreach($data_row as $row_data_key => $row_data_value)
		{
			...
			if(strpos($header_key_value, 'cf') !== false)
			{ //custom field
			...
			else
			{
				...
				{
					if($header_key_value == "contact_last_name")
					{
						$lead_contact_data["last_name"] = $row_data_value;
					}
					else
					{
						if($header_key_value == "contact_email")
						{
							$lead_contact_data["email"] = $row_data_value;
						}
						else
						{
							if($header_key_value == "status")
							{
								//get existing status, if not create new one and add the id
								$existing_status = $this->Lead_status_model->get_one_where(["title" => $row_data_value, "deleted" => 0]);
								...
								else
								{
									...
									$status_data = ["title" => $row_data_value, "color" => "#f1c40f", "sort" => ($max_sort_value * 1 + 1)];
									$lead_data["lead_status_id"] = $this->Lead_status_model->save($status_data);
								}
							}
							else
							{
								if($header_key_value == "source")
								{
									//get existing source, if not create new one and add the id
									$existing_source = $this->Lead_source_model->get_one_where(["title" => $row_data_value, "deleted" => 0]);
									...
									else
									{
										...
										$source_data = ["title" => $row_data_value, "sort" => ($max_sort_value * 1 + 1)];
	...
	private function _get_existing_custom_field_id($title = "")
	{
		...
		$existing = $this->Custom_fields_model->get_one_where(array_merge($custom_field_data, ["deleted" => 0]));
		if($existing->id)
		{
			return $existing->id;
		}
	...
	function save_lead_from_excel_file()
	{
		...
		{ //rows
			if($key === 0)
			{ //first line is headers, modify this for custom fields and continue for the next loop
	...
	private function _store_headers_position($headers_row = [])
	{
		$allowed_headers = $this->_get_allowed_headers();
		...
		foreach($headers_row as $key => $header)
		{
			...
			$header_array = ["key_value" => $header_on_this_position, "value" => $header];
			...
			{
				if((count($allowed_headers) - 1) < $key)
				{
	...
	function validate_import_leads_file()
	{
		...
		{
			echo json_encode(["success" => false, 'message' => lang('invalid_file_type')]);
			exit();
		}
		...
		{
			echo json_encode(["success" => false, 'message' => lang('please_upload_a_excel_file') . " (.xlsx)"]);
		}
	}
	...
	function validate_import_leads_file_data($check_on_submit = false)
	{
		...
		$table_data_header_array = [];
		$table_data_body_array = [];
		...
		foreach($excel_file as $row_key => $value)
		{
			...
			{ //validate headers
				...
				foreach($headers as $row_data)
				{
					...
					array_push($table_data_header_array, ["has_error_class" => $has_error_class, "value" => get_array_value($row_data, "value")]);
				}
			}
			else
			{ //validate data
				...
				foreach($value as $key => $row_data)
				{
					...
					$table_data_body_array[$row_key][] = ["has_error_class" => $has_error_class, "value" => $row_data];
				}
				...
				{
					$table_data_body_array[$row_key][] = ["has_error_text" => true, "value" => $error_message_on_this_row];
				}
			}
		}
		...
		if($got_error_table_data)
		{
			array_push($table_data_header_array, ["has_error_text" => true, "value" => lang("error")]);
		}
		...
		echo json_encode(["success" => true, 'table_data' => $table_data, 'got_error' => ($got_error_header || $got_error_table_data) ? true : false]);
	}
	...
	{
		$this->access_only_allowed_members();
		download_app_files(get_setting("system_file_path"), serialize([["file_name" => "import-leads-sample.xlsx"]]));
	}
