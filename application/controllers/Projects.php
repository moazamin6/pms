...
class Projects extends MY_Controller
{
	...
	private function can_delete_projects()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_delete_projects") == "1")
				{
					return true;
				}
			}
		...
		if($this->login_user->user_type == "staff")
		{
			...
			else
			{
				if(get_array_value($this->login_user->permissions, "show_assigned_tasks_only") !== "1")
				{
					...
					{
						if(get_array_value($this->login_user->permissions, "can_add_remove_project_members") == "1")
						{
							return true;
						}
					}
	...
	private function can_view_tasks($project_id = "", $task_id = "")
	{
		if($this->login_user->user_type == "staff")
		{
			...
			else
			{
				...
				{
					if($this->is_user_a_project_member)
					{
	...
	private function can_edit_tasks()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_edit_tasks") == "1")
				{
	...
	private function can_delete_tasks()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_delete_tasks") == "1")
				{
					//check is user a project member
					return $this->is_user_a_project_member;
				}
	...
	private function can_comment_on_tasks()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_comment_on_tasks") == "1")
				{
					//check is user a project member
					return $this->is_user_a_project_member;
				}
	...
	private function can_create_milestones()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_create_milestones") == "1")
				{
					//check is user a project member
					return $this->is_user_a_project_member;
				}
	...
	private function can_edit_milestones()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_edit_milestones") == "1")
				{
					//check is user a project member
					return $this->is_user_a_project_member;
				}
	...
	private function can_delete_milestones()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_delete_milestones") == "1")
				{
					//check is user a project member
					return $this->is_user_a_project_member;
				}
	...
	private function can_delete_files($uploaded_by = 0)
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_delete_files") == "1")
				{
					//check is user a project member
					return $this->is_user_a_project_member;
				}
	...
	{
		$settings = $this->Project_settings_model->get_all_where(["project_id" => $project_id])->result();
	...
	function save()
	{
		...
		if($save_id)
		{
			...
			if($status == "completed")
			{
				log_notification("project_completed", ["project_id" => $save_id]);
			}
			...
			if(!$id)
			{
				...
				if($estimate_id)
				{
					$data = ["project_id" => $save_id];
					$this->Estimates_model->save($data, $estimate_id);
				}
				
				log_notification("project_created", ["project_id" => $save_id]);
			}
	...
	function save_cloned_project()
	{
		...
		if($copy_milestones)
		{
			$milestones = $this->Milestones_model->get_all_where(["project_id" => $project_id, "deleted" => 0])->result();
		...
		$tasks = $this->Tasks_model->get_all_where(["project_id" => $project_id, "deleted" => 0, "parent_task_id" => 0])->result();
		...
		foreach($tasks as $task)
		{
			if(array_key_exists($task->id, $task_ids))
			{
				...
				if($task->blocked_by)
				{
					...
					if($new_blocked_by_tasks)
					{
						$blocked_by_task_data = ["blocked_by" => $new_blocked_by_tasks];
				...
				if($task->blocking)
				{
					...
					if($new_blocking_tasks)
					{
						$blocking_task_data = ["blocking" => $new_blocking_tasks];
		...
		//add project members
		$project_members = $this->Project_members_model->get_all_where(["project_id" => $project_id, "deleted" => 0])->result();
		...
		if($new_project_id)
		{
			...
			log_notification("project_created", ["project_id" => $new_project_id]);
			
			echo json_encode(["success" => true, 'id' => $new_project_id, 'message' => lang('project_cloned_successfully')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	{
		$old_custom_fields = $this->Custom_field_values_model->get_all_where(["related_to_type" => "tasks", "related_to_id" => $task->id, "deleted" => 0])->result();
	...
	function delete()
	{
		...
		{
			log_notification("project_deleted", ["project_id" => $id]);
	...
	private function _make_row($data, $custom_fields)
	{
		...
		if($this->can_edit_projects())
		{
			$optoins .= modal_anchor(get_uri("projects/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_project'), "data-post-id" => $data->id]);
		}
		
		if($this->can_delete_projects())
		{
			$optoins .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_project'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete"), "data-action" => "delete-confirmation"]);
		}
	...
	function overview($project_id)
	{
		...
		$view_data = $this->_get_project_info_data($project_id);
		$view_data["task_statuses"] = $this->Tasks_model->get_task_statistics(["project_id" => $project_id]);
		
		
		$view_data['project_id'] = $project_id;
		$offset = 0;
		$view_data['offset'] = $offset;
		$view_data['activity_logs_params'] = ["log_for" => "project", "log_for_id" => $project_id, "limit" => 20, "offset" => $offset];
	...
	private function can_access_clients()
	{
		if(get_setting("client_can_view_tasks"))
		{
			...
			{
				if($this->login_user->user_type == "staff" && get_array_value($this->login_user->permissions, "client") && get_array_value($this->login_user->permissions, "show_assigned_tasks_only") !== "1")
				{
					return true;
				}
			}
	...
	function overview_for_client($project_id)
	{
		if($this->login_user->user_type === "client")
		{
			...
			$view_data['activity_logs_params'] = [];
	...
	function save_project_member()
	{
		...
		if($user_ids)
		{
			foreach($user_ids as $user_id)
			{
				if($user_id)
				{
					...
					{
						$save_ids[] = $save_id;
						log_notification("project_member_added", ["project_id" => $project_id, "to_user_id" => $user_id]);
					}
					else
					{
						if($save_id === "exists")
		...
		else
		{
			if(count($save_ids))
			{
				...
				echo json_encode(["success" => true, "data" => $project_member_row, 'id' => $save_id, 'message' => lang('record_saved')]);
			}
			else
			{
				echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function delete_project_member()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_project_member_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
		...
		else
		{
			if($this->Project_members_model->delete($id))
			{
				...
				log_notification("project_member_deleted", ["project_id" => $project_member_info->project_id, "to_user_id" => $project_member_info->user_id]);
				echo json_encode(["success" => true, 'message' => lang('record_deleted')]);
			}
			else
			{
	...
	function project_member_list_data($project_id = 0, $user_type = "")
	{
		...
		$options = ["project_id" => $project_id, "user_type" => $user_type, "show_user_wise" => true];
	...
	private function _make_project_member_row($data, $can_send_message_to_client = false)
	{
		...
		{
			$link = modal_anchor(get_uri("messages/modal_form/" . $data->user_id), "<i class='fa fa-envelope-o'></i>", ["class" => "edit", "title" => lang('send_message')]);
		}
		...
		if($this->can_add_remove_project_members())
		{
			$delete_link = js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_member'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_project_member"), "data-action" => "delete"]);
	...
	{
		$tasks_dropdown = ["" => "-"];
		$tasks_dropdown_json = [["id" => "", "text" => "- " . lang("task") . " -"]];
		...
		foreach($tasks as $task)
		{
			$tasks_dropdown_json[] = ["id" => $task->id, "text" => $task->id . "-" . $task->title];
	...
	function timer($project_id, $timer_status = "start")
	{
		...
		if($timer_status === "start")
		{
			...
			{
				echo modal_anchor(get_uri("projects/stop_timer_modal_form/" . $project_id), "<i class='fa fa fa-clock-o'></i> " . lang('stop_timer'), ["class" => "btn btn-danger", "title" => lang('stop_timer'), "data-post-task_id" => $task_id]);
			}
			else
			{
				$view_data = $this->_get_project_info_data($project_id);
	...
	private function _get_project_members_dropdown_list_for_filter($project_id)
	{
		...
		$project_members_dropdown = [["id" => "", "text" => "- " . lang("member") . " -"]];
		foreach($project_members as $member)
		{
			$project_members_dropdown[] = ["id" => $member->user_id, "text" => $member->member_name];
		}
	...
	private function _prepare_all_related_data_for_timelog($project_id = 0)
	{
		...
		else
		{
			...
			$tasks_dropdown = json_encode([["id" => "", "text" => "-"]]);
			$project_members_dropdown = [["id" => "", "text" => "-"]];
	...
	function save_timelog()
	{
		...
		else
		{
			...
			if(!$hours)
			{
				echo json_encode(["success" => false, 'message' => lang("hour_log_time_error_message")]);
				return false;
			}
		}
		...
		if($save_id)
		{
			...
			echo json_encode(["success" => true, "data" => $this->_timesheet_row_data($save_id), 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function delete_timelog()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_timesheet_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
				echo json_encode(["success" => false, lang('error_occurred')]);
	...
	private function _make_timesheet_row($data, $custom_fields)
	{
		...
		$task_title = modal_anchor(get_uri("projects/task_view"), $data->task_title, ["title" => lang('task_info') . " #$data->task_id", "data-post-id" => $data->task_id, "data-modal-lg" => "1"]);
		...
		$note_link = modal_anchor(get_uri("projects/timer_note_modal_form/"), "<i class='fa fa-comment-o p10'></i>", ["class" => "edit text-muted", "title" => lang("note"), "data-post-id" => $data->id]);
		if($data->note)
		{
			$note_link = modal_anchor(get_uri("projects/timer_note_modal_form/"), "<i class='fa fa-comment p10'></i>", ["class" => "edit text-muted", "title" => $note_title, "data-modal-title" => lang("note"), "data-post-id" => $data->id]);
		}
		...
		$row_data[] = modal_anchor(get_uri("projects/timelog_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_timelog'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_timelog'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_timelog"), "data-action" => "delete"]);
	...
	function timesheet_summary($project_id)
	{
		...
		$view_data['group_by_dropdown'] = json_encode(
			[
				["id" => "", "text" => "- " . lang("group_by") . " -"],
				["id" => "member", "text" => lang("member")],
				["id" => "task", "text" => lang("task")],
			]);
	...
	{
		$projects = $this->Projects_model->get_dropdown_list(["title"]);
		
		$projects_dropdown = [["id" => "", "text" => "- " . lang("project") . " -"]];
		foreach($projects as $id => $title)
		{
			$projects_dropdown[] = ["id" => $id, "text" => $title];
		}
	...
	private function _get_members_to_manage_timesheet()
	{
		...
		else
		{
			...
			{
				return [$this->login_user->id]; //can access own timelogs
			}
		}
	}
	...
	private function _prepare_members_dropdown_for_timesheet_filter($members)
	{
		...
		if($members != "all")
		{
			$where["where_in"] = ["id" => $members];
		}
		
		$users = $this->Users_model->get_dropdown_list(["first_name", "last_name"], "id", $where);
	...
	function save_milestone()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_milestone_row_data($save_id), 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function delete_milestone()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_milestone_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
				echo json_encode(["success" => false, lang('error_occurred')]);
	...
	private function _make_milestone_row($data)
	{
		...
		{
			if($progress !== 100 && get_my_local_time("Y-m-d") > $data->due_date)
			{
		...
		if($this->can_edit_milestones())
		{
			$optoins .= modal_anchor(get_uri("projects/milestone_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_milestone'), "data-post-id" => $data->id]);
		}
		
		if($this->can_delete_milestones())
		{
			$optoins .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_milestone'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_milestone"), "data-action" => "delete"]);
		}
	...
	{
		$milestones = $this->Milestones_model->get_details(["project_id" => $project_id, "deleted" => 0])->result();
		$milestone_dropdown = [["id" => "", "text" => "- " . lang("milestone") . " -"]];
		
		foreach($milestones as $milestone)
		{
			$milestone_dropdown[] = ["id" => $milestone->id, "text" => $milestone->title];
		}
	...
	{
		$assigned_to_dropdown = [["id" => "", "text" => "- " . lang("assigned_to") . " -"]];
		$assigned_to_list = $this->Project_members_model->get_project_members_dropdown_list($project_id, [], true, true)->result();
		foreach($assigned_to_list as $assigned_to)
		{
			$assigned_to_dropdown[] = ["id" => $assigned_to->user_id, "text" => $assigned_to->member_name];
		}
	...
	function task_view($task_id = 0)
	{
		...
		$model_info = $this->Tasks_model->get_details(["id" => $task_id])->row();
		if(!$model_info->id)
		{
			show_404();
		}
		...
		$options = ["task_id" => $task_id, "login_user_id" => $this->login_user->id];
		...
		//get checklist items
		$checklist_items_array = [];
		$checklist_items = $this->Checklist_items_model->get_details(["task_id" => $task_id])->result();
		...
		//get sub tasks
		$sub_tasks_array = [];
		$sub_tasks = $this->Tasks_model->get_details(["parent_task_id" => $task_id])->result();
		...
		$timesheet_options = ["project_id" => $model_info->project_id, "task_id" => $model_info->id];
	...
	private function _initialize_all_related_data_of_project($project_id = 0, $collaborators = "", $task_labels = "")
	{
		...
		else
		{
			...
			$view_data['milestones_dropdown'] = [["id" => "", "text" => "-"]];
			$view_data['assign_to_dropdown'] = [["id" => "", "text" => "-"]];
			$view_data['collaborators_dropdown'] = [];
			$view_data['label_suggestions'] = [];
		}
	...
	function task_modal_form()
	{
		...
		$view_data['has_checklist'] = $this->Checklist_items_model->get_details(["task_id" => $id])->num_rows();
		$view_data['has_sub_task'] = $this->Tasks_model->get_all_where(["parent_task_id" => $id, "deleted" => 0])->num_rows();
	...
	private function get_all_related_data_of_project($project_id, $collaborators = "", $task_labels = "")
	{
		...
		if($project_id)
		{
			...
			$milestones = $this->Milestones_model->get_details(["project_id" => $project_id, "deleted" => 0])->result();
			$milestones_dropdown = [["id" => "", "text" => "-"]];
			foreach($milestones as $milestone)
			{
				$milestones_dropdown[] = ["id" => $milestone->id, "text" => $milestone->title];
			}
			...
			$collaborators_array = $collaborators ? explode(",", $collaborators) : [];
			foreach($project_members as $member)
			{
				$project_members_dropdown[] = ["id" => $member->user_id, "text" => $member->member_name];
				...
				{
					$collaborators_dropdown[] = ["id" => $member->user_id, "text" => $member->member_name];
				}
			}
	...
	function save_task()
	{
		...
		{
			echo json_encode(["success" => false, 'message' => lang('deadline_must_be_equal_or_greater_than_start_date')]);
			return false;
		}
		...
		if($recurring && get_setting("enable_recurring_option_for_tasks"))
		{
			...
			{
				echo json_encode(["success" => false, 'message' => lang('past_recurring_date_error_message_title_for_tasks'), 'next_recurring_date_error' => lang('past_recurring_date_error_message'), "next_recurring_date_value" => $next_recurring_date]);
				return false;
			}
		}
		...
		if($save_id)
		{
			...
			if($is_clone && $main_task_id)
			{
				...
				if($copy_checklist)
				{
					$checklist_items = $this->Checklist_items_model->get_all_where(["task_id" => $main_task_id, "deleted" => 0])->result();
					foreach($checklist_items as $checklist_item)
					{
				...
				{
					$sub_tasks = $this->Tasks_model->get_all_where(["parent_task_id" => $main_task_id, "deleted" => 0])->result();
					foreach($sub_tasks as $sub_task)
					{
			...
			{
				//updated
				log_notification("project_task_updated", ["project_id" => $project_id, "task_id" => $save_id, "activity_log_id" => $new_activity_log_id ? $new_activity_log_id : $activity_log_id]);
			}
			else
			{
				//created
				log_notification("project_task_created", ["project_id" => $project_id, "task_id" => $save_id]);
			...
			echo json_encode(["success" => true, "data" => $this->_task_row_data($save_id), 'id' => $save_id, 'message' => lang('record_saved'), "add_type" => $add_type]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function save_sub_task()
	{
		...
		if($save_id)
		{
			...
			$task_info = $this->Tasks_model->get_details(["id" => $save_id])->row();
			
			echo json_encode(["success" => true, "task_data" => $this->_make_sub_task_row($task_info), "data" => $this->_task_row_data($save_id), 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	private function _make_sub_task_row($data, $return_type = "row")
	{
		...
		if($this->can_edit_tasks())
		{
			$status = js_anchor("<span class='$checkbox_class'></span>", ['title' => "", "data-id" => $data->id, "data-value" => $data->status_key_name === "done" ? "1" : "3", "data-act" => "update-sub-task-status-checkbox"]);
		}
		
		$title = anchor(get_uri("projects/task_view/$data->id"), $data->title, ["class" => "font-13", "target" => "_blank"]);
	...
	function save_task_status($id = 0)
	{
		...
		$task_info = $this->Tasks_model->get_details(["id" => $id])->row();
		...
		{
			$task_info = $this->Tasks_model->get_details(["id" => $id])->row();
			echo json_encode(["success" => true, "data" => (($this->input->post("type") == "sub_task") ? $this->_make_sub_task_row($task_info, "data") : $this->_task_row_data($save_id)), 'id' => $save_id, "message" => lang('record_saved')]);
			
			log_notification("project_task_updated", ["project_id" => $task_info->project_id, "task_id" => $save_id, "activity_log_id" => get_array_value($data, "activity_log_id")]);
		}
		else
		{
			echo json_encode(["success" => false, lang('error_occurred')]);
	...
	function update_task_info($id = 0, $data_field = "")
	{
		if($id)
		{
			...
			if($save_id)
			{
				...
				$success_array = ["success" => true, "data" => $this->_task_row_data($save_id), 'id' => $save_id, "message" => lang('record_saved')];
	...
	function delete_task()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_task_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
				echo json_encode(["success" => false, lang('error_occurred')]);
		...
		else
		{
			if($this->Tasks_model->delete($id))
			{
				...
				$task_info = $this->Tasks_model->get_one($id);
				log_notification("project_task_deleted", ["project_id" => $task_info->project_id, "task_id" => $id]);
			}
			else
			{
				echo json_encode(["success" => false, 'message' => lang('record_cannot_be_deleted')]);
	...
	private function _make_task_row($data, $custom_fields)
	{
		...
		$title .= modal_anchor(get_uri("projects/task_view"), $data->title . $icon, ["title" => lang('task_info') . " #$data->id", "data-post-id" => $data->id, "class" => $unread_comments_class, "data-modal-lg" => "1"]);
		...
		if($this->login_user->user_type == "staff" && can_edit_this_task_status($data->assigned_to))
		{
			...
			$check_status = js_anchor("<span class='$checkbox_class'></span>", ['title' => "", "class" => "js-task", "data-id" => $data->id, "data-value" => $data->status_key_name === "done" ? "1" : "3", "data-act" => "update-task-status-checkbox"]) . $data->id;
			$status = js_anchor($data->status_key_name ? lang($data->status_key_name) : $data->status_title, ['title' => "", "class" => "", "data-id" => $data->id, "data-value" => $data->status_id, "data-act" => "update-task-status"]);
		}
		else
		{
		...
		if($this->can_edit_tasks())
		{
			$options .= modal_anchor(get_uri("projects/task_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_task'), "data-post-id" => $data->id]);
		}
		if($this->can_delete_tasks())
		{
			$options .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_task'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_task"), "data-action" => "delete"]);
		}
		...
		if($collaborator_list)
		{
			...
			foreach($collaborators_array as $collaborator)
			{
				...
				if($clickable)
				{
					if($user_type == "staff")
					{
						$collaborators .= get_team_member_profile_link($collaborator_id, $collaboratr_image, ["title" => $collaborator_name]);
					}
					else
					{
						if($user_type == "client")
						{
							$collaborators .= get_client_contact_profile_link($collaborator_id, $collaboratr_image, ["title" => $collaborator_name]);
						}
					}
				}
				else
	...
	function comments($project_id)
	{
		...
		$options = ["project_id" => $project_id, "login_user_id" => $this->login_user->id];
		$view_data['comments'] = $this->Project_comments_model->get_details($options)->result();
		$view_data['project_id'] = $project_id;
	...
	{
		$options = ["customer_feedback_id" => $project_id, "login_user_id" => $this->login_user->id]; //customer feedback id and project id is same
		$view_data['comments'] = $this->Project_comments_model->get_details($options)->result();
	...
	function save_comment()
	{
		...
		{
			$response_data = "";
			$options = ["id" => $save_id, "login_user_id" => $this->login_user->id];
			...
			$notification_options = ["project_id" => $comment_info->project_id, "project_comment_id" => $save_id];
			...
			else
			{
				...
				{
					if($comment_info->customer_feedback_id)
	...
	{
		$view_data['reply_list'] = $this->Project_comments_model->get_details(["comment_id" => $comment_id])->result();
	...
	{
		$file_info = $this->Project_files_model->get_details(["id" => $file_id])->row();
		...
		if($file_info)
		{
			...
			$options = ["file_id" => $file_id, "login_user_id" => $this->login_user->id];
			$view_data['comments'] = $this->Project_comments_model->get_details($options)->result();
	...
	function save_file()
	{
		...
		if($files && get_array_value($files, 0))
		{
			foreach($files as $file)
			{
				...
				if($file_info)
				{
					...
					log_notification("project_file_added", ["project_id" => $project_id, "project_file_id" => $success]);
				}
				else
				{
					$success = false;
		...
		if($success)
		{
			echo json_encode(["success" => true, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function delete_file()
	{
		...
		if($this->Project_files_model->delete($id))
		{
			...
			delete_app_files($file_path . $info->project_id . "/", [make_array_of_file($info)]);
			
			log_notification("project_file_deleted", ["project_id" => $info->project_id, "project_file_id" => $id]);
			echo json_encode(["success" => true, 'message' => lang('record_deleted')]);
		}
		else
		{
	...
	function download_file($id)
	{
		...
		//serilize the path
		$file_data = serialize([["file_name" => $file_info->project_id . "/" . $file_info->file_name, "file_id" => $file_info->file_id, "service_type" => $file_info->service_type]]);
		...
		if($files_ids)
		{
			...
			if($files)
			{
				...
				foreach($files->result() as $file_info)
				{
					...
					$file_path_array[] = ["file_name" => $file_info->project_id . "/" . $file_info->file_name, "file_id" => $file_info->file_id, "service_type" => $file_info->service_type];
				}
	...
	function save_batch_update()
	{
			...
			if($task_ids)
			{
				...
				foreach($tasks_ids_array as $id)
				{
					...
					if($save_id)
					{
						...
						if($activity_log_id)
						{
							log_notification("project_task_updated", ["project_id" => $project_id, "task_id" => $save_id, "activity_log_id" => $activity_log_id]);
						}
					}
				}
		...
		{
			echo json_encode(['success' => false, 'message' => lang('no_field_has_selected')]);
			return false;
		}
	}
	...
	function download_comment_files($id)
	{
		...
		{
			if($this->login_user->user_type == "user" && !$this->can_view_tasks())
			{
				redirect("forbidden");
			}
		}
	...
	private function _make_file_row($data)
	{
		...
		$description = "<div class='pull-left'>" .
			js_anchor(remove_file_prefix($data->file_name), ['title' => "", "data-toggle" => "app-modal", "data-sidebar" => "1", "data-url" => get_uri("projects/view_file/" . $data->id)]);
		...
		$options = anchor(get_uri("projects/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", ["title" => lang("download")]);
		if($this->can_delete_files($data->uploaded_by))
		{
			$options .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("projects/delete_file"), "data-action" => "delete-confirmation"]);
		}
		
		//show checkmark to download multiple files
		$checkmark = js_anchor("<span class='checkbox-blank'></span>", ['title' => "", "class" => "", "data-id" => $data->id, "data-act" => "download-multiple-file-checkbox"]) . $data->id;
	...
	function history($offset = 0, $log_for = "", $log_for_id = "", $log_type = "", $log_type_id = "")
	{
		...
		$view_data['offset'] = $offset;
		$view_data['activity_logs_params'] = ["log_for" => $log_for, "log_for_id" => $log_for_id, "log_type" => $log_type, "log_type_id" => $log_type_id, "limit" => 20, "offset" => $offset];
	...
	function payments($project_id)
	{
		...
		if($project_id)
		{
			$view_data['project_info'] = $this->Projects_model->get_details(["id" => $project_id])->row();
			$view_data['project_id'] = $project_id;
	...
	function gantt($project_id = 0)
	{
		...
		if($project_id)
		{
			...
			{
				$status_dropdown[] = ["id" => $status->id, "text" => ($status->key_name ? lang($status->key_name) : $status->title)];
			}
	...
	function save_settings()
	{
		...
		if($can_edit_slack_settings && $this->input->post("send_a_test_message"))
		{
			...
			{
				echo json_encode(["success" => true, 'message' => lang('settings_updated')]);
			}
			else
			{
				echo json_encode(["success" => false, 'message' => lang('slack_notification_error_message')]);
			}
		}
		else
		{
	...
	function save_checklist_item()
	{
		...
		if($save_id)
		{
			...
			echo json_encode(["success" => true, "data" => $this->_make_checklist_item_row($item_info), 'id' => $save_id]);
		}
		else
		{
			echo json_encode(["success" => false]);
	...
	private function _make_checklist_item_row($data = [], $return_type = "row")
	{
		...
		$status = js_anchor("<span class='$checkbox_class'></span>", ['title' => "", "data-id" => $data->id, "data-value" => $is_checked_value, "data-act" => "update-checklist-item-status-checkbox"]);
		if(!$this->can_edit_tasks())
		{
			$status = "";
		}
		...
		$delete = ajax_anchor(get_uri("projects/delete_checklist_item/$data->id"), "<i class='fa fa-times pull-right p3'></i>", ["class" => "delete-checklist-item", "title" => lang("delete_checklist_item"), "data-fade-out-on-success" => "#checklist-item-row-$data->id"]);
		if(!$this->can_edit_tasks())
		{
			$delete = "";
		}
	...
	function save_checklist_item_status($id = 0)
	{
		...
		{
			$item_info = $this->Checklist_items_model->get_one($save_id);
			echo json_encode(["success" => true, "data" => $this->_make_checklist_item_row($item_info, "data"), 'id' => $save_id]);
		}
		else
		{
			echo json_encode(["success" => false]);
	...
	function get_member_suggestion_to_mention()
	{
		...
		foreach($project_members as $member)
		{
			$project_members_dropdown[] = ["name" => $member->member_name, "content" => "@[" . $member->member_name . " :" . $member->user_id . "]"];
		}
		
		if($project_members_dropdown)
		{
			echo json_encode(["success" => true, "data" => $project_members_dropdown]);
		}
		else
		{
			echo json_encode(["success" => false]);
	...
	function get_projects_of_selected_client_for_filter()
	{
		...
		if($client_id)
		{
			$projects = $this->Projects_model->get_all_where(["client_id" => $client_id, "deleted" => 0], "", "", "title")->result();
			$projects_dropdown = [["id" => "", "text" => "- " . lang("project") . " -"]];
			foreach($projects as $project)
			{
				$projects_dropdown[] = ["id" => $project->id, "text" => $project->title];
	...
	function all_gantt()
	{
		...
		$team_members_dropdown = [["id" => "", "text" => "- " . lang("assigned_to") . " -"]];
		$assigned_to_list = $this->Users_model->get_dropdown_list(["first_name", "last_name"], "id", ["deleted" => 0, "user_type" => "staff"]);
		foreach($assigned_to_list as $key => $value)
		{
			$team_members_dropdown[] = ["id" => $key, "text" => $value];
	...
	function timesheet_chart_data($project_id = 0)
	{
		...
		echo json_encode(["timesheets" => $timesheets_array, "ticks" => $ticks]);
	}
	...
	function save_dependency_tasks()
	{
		...
		if($task_id)
		{
			...
			if($dependency_task)
			{
				...
				$dependency_task_info = $this->Tasks_model->get_details(["id" => $dependency_task])->row();
				
				echo json_encode(["success" => true, "data" => $this->_make_dependency_tasks_row_data($dependency_task_info, $task_id, $dependency_type), 'message' => lang('record_saved')]);
			}
		}
	}
	...
	function get_existing_dependency_tasks($task_id = 0)
	{
		if($task_id)
		{
			...
			//make tasks dropdown
			$tasks_dropdown = [];
			$tasks = $this->Tasks_model->get_details(["project_id" => $model_info->project_id, "exclude_task_ids" => $all_dependency_tasks])->result();
			foreach($tasks as $task)
			{
				$tasks_dropdown[] = ["id" => $task->id, "text" => $task->id . " - " . $task->title];
			}
			
			echo json_encode(["success" => true, "tasks_dropdown" => $tasks_dropdown]);
		}
	}
	...
	private function _make_dependency_tasks_view_data($task_ids = "", $task_id = 0, $type = "")
	{
		if($task_ids)
		{
			...
			$tasks_list = $this->Tasks_model->get_details(["task_ids" => $task_ids])->result();
	...
	private function _make_dependency_tasks_row_data($task_info, $task_id, $type)
	{
		...
		if($this->can_edit_tasks())
		{
			$tasks .= ajax_anchor(get_uri("projects/delete_dependency_task/$task_info->id/$task_id/$type"), "<i class='fa fa-times pull-right p3'></i>", ["class" => "delete-dependency-task", "title" => lang("delete"), "data-fade-out-on-success" => "#dependency-task-row-$task_info->id", "data-dependency-type" => $type]);
		}
		
		$tasks .= modal_anchor(get_uri("projects/task_view"), $task_info->title, ["data-post-id" => $task_info->id, "data-modal-lg" => "1"]);
	...
	function delete_dependency_task($dependency_task_id, $task_id, $type)
	{
		...
		{
			if(!strpos($dependency_tasks_of_others, ',') && $dependency_tasks_of_others == $task_id)
			{
				$data = [(($type == "blocked_by") ? "blocking" : "blocked_by") => ""];
			...
			else
			{
				...
				if(in_array($dependency_task_id, $dependency_tasks_of_own_array))
				{
					...
					$data = [$type => $dependency_tasks_of_own_array];
					$this->Tasks_model->update_custom_data($data, $task_id);
				}
				else
				{
					if(in_array($task_id, $dependency_tasks_of_others_array))
					{
						...
						$data = [(($type == "blocked_by") ? "blocking" : "blocked_by") => $dependency_tasks_of_others_array];
						$this->Tasks_model->update_custom_data($data, $dependency_task_id);
					}
				}
			}
	...
	function like_comment($comment_id = 0)
	{
		if($comment_id)
		{
			...
			$existing = $this->Likes_model->get_one_where(array_merge($data, ["deleted" => 0]));
			if($existing->id)
			{
			...
			$options = ["id" => $comment_id, "login_user_id" => $this->login_user->id];
			$comment = $this->Project_comments_model->get_details($options)->row();
			
			$this->load->view("projects/comments/like_comment", ["comment" => $comment]);
		}
	}
