...
class Notes extends MY_Controller
{
	...
	private function _make_row($data)
	{
		...
		$title = modal_anchor(get_uri("notes/view/" . $data->id), $public_icon . $data->title, ["title" => lang('note'), "data-post-id" => $data->id]);
		...
		if($data->files)
		{
			...
			if(count($files))
			{
				foreach($files as $key => $value)
				{
					...
					$files_link .= js_anchor(" ", ['title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "pull-left font-22 mr10 $link", "title" => remove_file_prefix($file_name), "data-url" => get_uri("notes/file_preview/" . $data->id . "/" . $key)]);
				}
			}
		}

		//only creator and admin can edit/delete notes
		$actions = modal_anchor(get_uri("notes/view/" . $data->id), "<i class='fa fa-bolt'></i>", ["class" => "edit", "title" => lang('note_details'), "data-modal-title" => lang("note"), "data-post-id" => $data->id]);
		if($data->created_by == $this->login_user->id || $this->login_user->is_admin)
		{
			$actions = modal_anchor(get_uri("notes/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_note'), "data-post-id" => $data->id])
				. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_note'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("notes/delete"), "data-action" => "delete-confirmation"]);
		}
	...
	function view()
	{
		...
		$model_info = $this->Notes_model->get_details(["id" => $this->input->post('id')])->row();
