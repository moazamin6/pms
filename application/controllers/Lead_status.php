...
class Lead_status extends MY_Controller
{
	...
	{
		$edit = modal_anchor(get_uri("lead_status/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_lead_status'), "data-post-id" => $data->id]);

		$delete_attributes = ['title' => lang('delete_lead_status'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("lead_status/delete"), "data-action" => "delete-confirmation"];
		if($data->total_leads)
		{
			//the status shouldn't be deleted if there is any leads
			$delete_attributes = ['title' => lang('there_has_leads_with_this_status'), "class" => "delete not-clickable text-off",];
		}
