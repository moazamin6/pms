...
class Request_estimate extends CI_Controller
{
	...
	function index()
	{
		...
		$view_data["estimate_forms"] = $this->Estimate_forms_model->get_all_where(["status" => "active", "public" => "1", "deleted" => 0])->result();
	...
	function form($id = 0, $embedded = 0)
	{
		...
		$model_info = $this->Estimate_forms_model->get_one_where(["id" => $id, "public" => "1", "status" => "active", "deleted" => 0]);
	...
	function save_estimate_request()
	{
		...
		if(get_setting("re_captcha_secret_key"))
		{
			...
			if($response !== true)
			{
				...
				if($response)
				{
					echo json_encode(['success' => false, 'message' => lang("re_captcha_error-" . $response)]);
				}
				else
				{
					echo json_encode(['success' => false, 'message' => lang("re_captcha_expired")]);
				}
		...
		if($save_id)
		{
			...
			echo json_encode(["success" => true, 'message' => lang('estimate_submission_message')]);
		}
		else
		{
	...
	private function _make_form_field_row($data)
	{
		...
		return [
			...
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("estimate_requests/estimate_form_field_delete"), "data-action" => "delete"]),
		];
	}
