...
class Signup extends CI_Controller
{
	...
	function create_account()
	{
		...
		if($signup_key)
		{
			...
			if($valid_key)
			{
				...
				if($this->Users_model->is_email_exists($email))
				{
					echo json_encode(["success" => false, 'message' => lang("account_already_exists_for_your_mail") . " " . anchor("signin", lang("signin"))]);
					return false;
				}
				...
				else
				{
					...
					if(isset($client->id) && $client->deleted == 0)
					{
						...
						if($user_id)
						{
							log_notification("invited_client_contact_signed_up", ["client_id" => $clent_id], $user_id);
						}
					}
					else
					{
				...
				{
					$options = ["code" => $signup_key, "type" => "invitation"];
			...
			else
			{
				...
				echo json_encode(["success" => false, 'message' => lang("invitation_expaired_message")]);
				return false;
			}
		}
		else
		{
			...
			{
				echo json_encode(["success" => false, 'message' => lang("account_already_exists_for_your_company_name") . " " . anchor("signin", lang("signin"))]);
				return false;
			}
			...
			if($client_id)
			{
				...
				if($verify_email_key)
				{
					$options = ["code" => $verify_email_key, "type" => "verify_email"];
					$verification_info = $this->Verification_model->get_details($options)->row();
					if($verification_info->id)
					{
						$this->Verification_model->delete_permanently($verification_info->id);
		...
		if($user_id)
		{
			echo json_encode(["success" => true, 'message' => lang('account_created') . " " . anchor("signin", lang("signin"))]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function send_verification_mail()
	{
		...
		{
			echo json_encode(['success' => true, 'message' => lang("reset_info_send")]);
		}
		else
		{
			echo json_encode(['success' => false, 'message' => lang('error_occurred')]);
	...
	private function is_valid_email_verification_key($verification_code = "")
	{
		...
		{
			$options = ["code" => $verification_code, "type" => "verify_email"];
			$verification_info = $this->Verification_model->get_details($options)->row();
	...
	private function is_valid_invitation_key($verification_code = "")
	{
		if($verification_code)
		{
			$options = ["code" => $verification_code, "type" => "invitation"];
			$verification_info = $this->Verification_model->get_details($options)->row();
			...
			if($verification_info && $verification_info->id)
			{
				...
				if($email && valid_email($email) && $expire_time && $expire_time > time())
				{
					return ["email" => $email, "type" => $type, "client_id" => $client_id];
				}
			}
		}
	}
