...
class Updates extends MY_Controller
{
	...
	function download_updates($version = "", $salt = "")
	{
		...
		if(is_file($update_zip))
		{
			echo json_encode(["success" => true, 'message' => "File already exists"]);
		}
		else
		{
			...
			if($new_update)
			{
				...
				if(!is_dir($local_updates_dir))
				{
					...
					{
						echo json_encode(["success" => false, 'message' => "Permission denied: $local_updates_dir directory is not writeable! Please set the writeable permission to the directory"]);
						exit();
					}
				}
				...
				{
					echo json_encode(["success" => true, 'message' => "Downloaded version-" . $version]);
				}
				else
				{
			...
			{
				echo json_encode(["success" => false, 'message' => "Sorry, Version - $version download has been failed!"]);
			}
		}
	}
	...
	function do_update($version = "")
	{
		...
		if($version)
		{
			...
			{
				echo json_encode(["success" => false, 'message' => "Please install the version - $updates_info->next_installable_version first!"]);
				exit();
			}
			$local_updates_dir = get_setting("updates_path");
			...
			{
				echo json_encode(["success" => false, 'message' => "Please instal the zip extension in your server."]);
				exit();
			}
			...
			echo json_encode(["success" => true, 'message' => "Version - $version installed successfully!"]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang("something_went_wrong")]);
