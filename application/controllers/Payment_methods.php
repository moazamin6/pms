...
class Payment_methods extends MY_Controller
{
	...
	{
		$options = modal_anchor(get_uri("payment_methods/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_payment_method'), "data-post-id" => $data->id]);

		if(!$data->online_payable)
		{
			$options .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_payment_method'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("payment_methods/delete"), "data-action" => "delete"]);
		}
