...
class External_tickets extends CI_Controller
{
	...
	function save()
	{
		...
		$contact_info = $this->Users_model->get_one_where(["email" => $email, "user_type" => "client", "deleted" => 0]);
		...
		if($ticket_id)
		{
			...
			if($ticket_id && $ticket_comment_id)
			{
				...
				log_notification("ticket_created", ["ticket_id" => $ticket_id, "ticket_comment_id" => $ticket_comment_id, "exclude_ticket_creator" => true], $contact_info->id ? $contact_info->id : "0");

				echo json_encode(["success" => true, 'message' => lang('ticket_submission_message')]);
