...
class Help extends MY_Controller
{
	...
	function index()
	{
		...
		$view_data["categories"] = $this->Help_categories_model->get_details(["type" => $type, "only_active_categories" => true])->result();
		$view_data["type"] = $type;
	...
	function view($id = 0)
	{
		...
		$model_info = $this->Help_articles_model->get_details(["id" => $id])->row();
		...
		$view_data['categories'] = $this->Help_categories_model->get_details(["type" => $model_info->type])->result();
	...
	function category($id)
	{
		...
		$view_data['categories'] = $this->Help_categories_model->get_details(["type" => $category_info->type])->result();
	...
	function save_category()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_category_row_data($save_id), 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	function delete_category()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_category_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
	...
	function categories_list_data($type)
	{
		...
		$list_data = $this->Help_categories_model->get_details(["type" => $type])->result();
		$result = [];
	...
	private function _make_category_row($data)
	{
		return [
			...
			lang($data->status),
			$data->sort,
			modal_anchor(get_uri("help/category_modal_form/" . $data->type), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_category'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_category'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("help/delete_category"), "data-action" => "delete"]),
		];
	}
	...
	function article_form($type, $id = 0)
	{
		...
		$view_data['type'] = $type;
		$view_data['categories_dropdown'] = $this->Help_categories_model->get_dropdown_list(["title"], "id", ["type" => $type]);
	...
	function delete_article()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_article_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
				echo json_encode(["success" => false, lang('error_occurred')]);
	...
	function articles_list_data($type)
	{
		...
		$list_data = $this->Help_articles_model->get_details(["type" => $type])->result();
		$result = [];
		foreach($list_data as $data)
		{
	...
	private function _make_article_row($data)
	{
		return [
			...
			lang($data->status),
			$data->total_views,
			anchor(get_uri("help/article_form/" . $data->type . "/" . $data->id), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_article')])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_article'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("help/delete_article"), "data-action" => "delete"]),
		];
	}
