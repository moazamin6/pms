...
class Messages extends MY_Controller
{
	...
	private function check_validate_sending_message($to_user_id)
	{
		...
		{
			echo json_encode(["success" => false, 'message' => lang("message_sending_error_message")]);
			exit;
		}
	}
	...
	function list_data($mode = "inbox")
	{
		...
		$options = ["user_id" => $this->login_user->id, "mode" => $mode, "user_ids" => $this->get_allowed_user_ids()];
	...
	function view($message_id = 0, $mode = "", $reply = 0)
	{
		...
		{
			if($reply == 1 && $mode == "sent_items")
			{
				$message_mode = "inbox";
			}
		}
		
		$options = ["id" => $message_id, "user_id" => $this->login_user->id, "mode" => $message_mode];
		...
		$replies_options = ["message_id" => $message_id, "user_id" => $this->login_user->id, "limit" => 4];
		...
		echo json_encode(["success" => true, "data" => $this->load->view("messages/view", $view_data, true), "message_id" => $message_id]);
	}
	...
	function reply($is_chat = 0)
	{
		...
		if($message_info->id)
		{
			...
			if($save_id)
			{
				...
				if($is_chat)
				{
					echo json_encode(["success" => true, 'data' => $this->_load_messages($message_id, $this->input->post("last_message_id"), 0, true, $to_user_id)]);
				}
				else
				{
					$options = ["id" => $save_id, "user_id" => $this->login_user->id];
					$view_data['reply_info'] = $this->Messages_model->get_details($options)->row;
					echo json_encode(["success" => true, 'message' => lang('message_sent'), 'data' => $this->load->view("messages/reply_row", $view_data, true)]);
				}
	...
	private function _load_more_messages($message_id, $last_message_id, $top_message_id, $load_as_data = false)
	{
		...
		$replies_options = ["message_id" => $message_id, "last_message_id" => $last_message_id, "top_message_id" => $top_message_id, "user_id" => $this->login_user->id, "limit" => 10];
	...
	function get_notifications()
	{
		...
		echo json_encode(["success" => true, "active_message_id" => $this->input->post("active_message_id"), 'total_notifications' => $notifiations->num_rows(), 'notification_list' => $this->load->view("messages/notifications", $view_data, true)]);
	}
	...
	function chat_list()
	{
		...
		$options = ["login_user_id" => $this->login_user->id, "user_ids" => $this->get_allowed_user_ids()];
	...
	function view_chat()
	{
		...
		{
			$view_data["first_message"] = $this->Messages_model->get_details(["id" => $message_id, "user_id" => $this->login_user->id])->row;
	...
	private function _load_messages($message_id, $last_message_id, $top_message_id, $load_as_data = false, $another_user_id = "")
	{
		...
		$replies_options = ["message_id" => $message_id, "last_message_id" => $last_message_id, "top_message_id" => $top_message_id, "user_id" => $this->login_user->id];
	...
	function get_chatlist_of_user()
	{
		...
		$user_info = $this->Users_model->get_one_where(["id" => $user_id, "status" => "active", "deleted" => "0"]);
