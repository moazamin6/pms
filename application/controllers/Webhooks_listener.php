			...
			foreach($final_commits_array as $commit)
			{
				...
				if($task_info->id)
				{
					$log_data = [
						...
						"log_type_id"    => $task_id,
						"changes"        => serialize(["bitbucket" => ["from" => "", "to" => $commit]]),
						"log_for"        => "project",
						"log_for_id"     => $task_info->project_id,
					];

					$save_id = $this->Activity_logs_model->save($log_data, true);

					if($save_id)
					{
						//send notification
						$notification_options = ["project_id" => $task_info->project_id, "task_id" => $task_id, "activity_log_id" => $save_id, "user_id" => "999999998"];
	...
	private function _get_final_commits_of_bitbucket($payloads)
	{
		...
		if($changes)
		{
			...
			foreach($commits as $commit)
			{
				...
				{
					array_push($commits_description, [
						"task_id"        => $task_id,
			...
			foreach($commits_description as $key => $value)
			{
				...
				if(is_int($task_id) && $task_id)
				{
					...
					{
						array_push($final_commits_array, [
					...
					else
					{
						...
						array_push($final_commits_array[$commit]["commits"], [
							"commit_url"     => get_array_value($value, "commit_url"),
							"commit_message" => get_array_value($value, "commit_message"),
						]);
					}
			...
			foreach($final_commits_array as $commit)
			{
				...
				if($task_info->id)
				{
					$log_data = [
						...
						"log_type_title" => $task_info->title,
						"log_type_id"    => $task_id,
						"changes"        => serialize(["github" => ["from" => "", "to" => $commit]]),
						"log_for"        => "project",
						"log_for_id"     => $task_info->project_id,
					];
					...
					{
						//send notification
						$notification_options = ["project_id" => $task_info->project_id, "task_id" => $task_id, "activity_log_id" => $save_id, "user_id" => "999999997"];
...
/* Location: ./application/controllers/Webhooks_listener.php */    