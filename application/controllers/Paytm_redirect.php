...
class Paytm_redirect extends CI_Controller
{
	...
	function index($payment_verification_code = "")
	{
		...
		$options = ["code" => $payment_verification_code, "type" => "invoice_payment"];
		...
		if(get_array_value($data_array, "STATUS") !== "TXN_SUCCESS")
		{
			...
			$this->session->set_flashdata("error_message", get_array_value($data_array, "RESPMSG"));
			$this->redirect_to_invoice($invoice_id, $verification_code);
		}

		//validate the checksum hash
		$is_valid_checksum_hash = $this->paytm->is_valid_checksum_hash($data_array);
		...
		$existing = $this->Invoice_payments_model->get_one_where(["transaction_id" => get_array_value($data_array, "TXNID")]);
		if($existing->id)
		{
			show_404();
		}
...
/* Location: ./application/controllers/Paytm_redirect.php */