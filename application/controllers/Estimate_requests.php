...
class Estimate_requests extends MY_Controller
{
	...
	function index()
	{
		...
		{
			$assigned_to_dropdown[] = ["id" => $key, "text" => $value];
		}
		...
		$statuses_dropdown = [
			["id" => "", "text" => "- " . lang("status") . " -"],
			["id" => "new", "text" => lang("new")],
			["id" => "processing", "text" => lang("processing")],
			["id" => "estimated", "text" => lang("estimated")],
			["id" => "hold", "text" => lang("hold")],
			["id" => "canceled", "text" => lang("canceled")],
		];
	...
	function view_estimate_request($id = 0)
	{
		...
		$model_info = $this->Estimate_requests_model->get_details(["id" => $id])->row();
		...
		{
			$view_data['lead_info'] = $this->Clients_model->get_details(["id" => $model_info->client_id])->row();
		}
		...
		$view_data["estimates"] = $this->Estimates_model->get_all_where(["estimate_request_id" => $id])->result();
	...
	function estimate_request_list_data()
	{
		...
		$options = ["assigned_to" => $this->input->post("assigned_to"), "status" => $this->input->post("status")];
	...
	function estimate_requests_list_data_of_client($client_id)
	{
		...
		$options = ["client_id" => $client_id, "status" => $this->input->post("status")];
		$list_data = $this->Estimate_requests_model->get_details($options)->result();
		$result = [];
		foreach($list_data as $data)
		{
	...
	private function _make_estimate_request_row($data)
	{
		...
		$edit = '<li role="presentation">' . modal_anchor(get_uri("estimate_requests/edit_estimate_request_modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit'), ["title" => lang('estimate_request'), "data-post-view" => "details", "data-post-id" => $data->id]) . '</li>';
		
		$request_status = $this->load->view("estimate_requests/estimate_request_status_options", ["model_info" => $data], true);
		
		$add_estimate = '<li role="presentation">' . modal_anchor(get_uri("estimates/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_estimate'), ['title' => lang('add_estimate'), "data-post-estimate_request_id" => $data->id, "data-post-client_id" => $data->client_id]) . '</li>';
		
		$delete = '<li role="presentation">' . js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), ['title' => lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("estimate_requests/delete_estimate_request"), "data-action" => "delete-confirmation"]) . '</li>';
	...
	function estimate_request_filed_list_data($id = 0)
	{
		...
		$options = ["related_to_type" => "estimate_request", "related_to_id" => $id];
	...
	private function _make_estimate_request_field_row($data)
	{
		...
		$field .= "<div class='pl15'>" . $this->load->view("custom_fields/output_" . $data->custom_field_type, ["value" => $data->value], true) . "</div>";
		return [
			$field,
			$data->sort,
		];
	...
	function estimate_request_modal_form()
	{
		...
		$assigned_to_dropdown = ["" => "-"] + $this->Users_model->get_dropdown_list(["first_name", "last_name"], "id", ["deleted" => 0, "user_type" => "staff"]);
	...
	function save_estimate_request_form()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_form_row_data($save_id), 'newData' => $id ? false : true, 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	function delete_estimate_request_form()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_form_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
	...
	{
		$title = anchor(get_uri("estimate_requests/edit_estimate_form/" . $data->id), $data->title, ["class" => "edit", "title" => lang('edit_estimate_form'), "data-post-id" => $data->id]);
		
		$public = "";
		if($data->public)
		{
			$public = anchor("request_estimate/form/" . $data->id, lang("yes") . "<i class='fa fa-external-link ml10'></i>", ["target" => "_blank", "class" => ""]);
		}
		else
		{
			$public = lang("no");
		...
		if($data->public)
		{
			$embedded_code = modal_anchor(get_uri("estimate_requests/embedded_code_modal_form"), "<i class='fa fa-code'></i>", ["title" => lang('embed'), "class" => "edit", "data-post-id" => $data->id]);
		}
		...
		return [
			...
			$embedded_code,
			lang($data->status),
			modal_anchor(get_uri("estimate_requests/estimate_request_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_form'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("estimate_requests/delete_estimate_request_form"), "data-action" => "delete"]),
		];
	}
	...
	function save_estimate_form_field()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_form_filed_row_data($save_id), 'newData' => $id ? false : true, 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	private function _make_form_field_row($data)
	{
		...
		$field .= "<div class='form-group'>" . $this->load->view("custom_fields/input_" . $data->field_type, ["field_info" => $data], true) . "</div>";
		...
		return [
			$field,
			$data->sort,
			modal_anchor(get_uri("estimate_requests/estimate_form_field_modal_form/" . $estimate_form_id), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_form'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_estimate_form'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("estimate_requests/estimate_form_field_delete"), "data-action" => "delete"]),
		];
	}
	...
	function estimate_form_field_delete()
	{
		...
		if($this->input->post('undo'))
		{
			...
			{
				echo json_encode(["success" => true, "data" => $this->_form_filed_row_data($id), "message" => lang('record_undone')]);
			}
			else
			{
				echo json_encode(["success" => false, lang('error_occurred')]);
	...
	function request_an_estimate_modal_form()
	{
		...
		$view_data["estimate_forms"] = $this->Estimate_forms_model->get_all_where(["status" => "active", "deleted" => 0])->result();
	...
	function submit_estimate_request_form($id = 0)
	{
		...
		$model_info = $this->Estimate_forms_model->get_one_where(["id" => $id, "status" => "active", "deleted" => 0]);
	...
	function save_estimate_request()
	{
		...
		if($save_id)
		{
			...
			log_notification("estimate_request_received", ["estimate_request_id" => $save_id]);
			
			$this->session->set_flashdata("success_message", lang("estimate_submission_message"));
			
			echo json_encode(["success" => true, 'message' => lang('estimate_submission_message'), 'estimate_id' => $save_id]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	{
		$model_info = $this->Estimate_forms_model->get_one($this->input->post('id'));
		...
		$this->load->view('estimate_requests/embedded_code_modal_form', $view_data);
	}
