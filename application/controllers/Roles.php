...
class roles extends MY_Controller
{
	...
	function permissions($role_id)
	{
		if($role_id)
		{
			...
			$ticket_types = $this->Ticket_types_model->get_all_where(["deleted" => 0])->result();
	...
	function save_permissions()
	{
		...
		{
			if($this->input->post('message_permission_specific_checkbox'))
			{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_row_data($id), 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	private function _make_row($data)
	{
		return [
			...
			"<a class='edit'><i class='fa fa-check' ></i></a>" . modal_anchor(get_uri("roles/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "", "title" => lang('edit_role'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_role'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("roles/delete"), "data-action" => "delete"]),
		];
	}
