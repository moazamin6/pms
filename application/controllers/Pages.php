...
class Pages extends MY_Controller
{
	...
	function save()
	{
		...
		{
			echo json_encode(["success" => false, 'message' => lang("page_url_cant_duplicate")]);
			return false;
		}
	...
	{
		$options = modal_anchor(get_uri("pages/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('add_page'), "data-post-id" => $data->id]);

		$options .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_page'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("pages/delete"), "data-action" => "delete"]);

		return [
			$data->title,
			anchor(get_uri("about") . "/" . $data->slug, get_uri("about") . "/" . $data->slug, ["target" => "_blank"]),
			lang($data->status),
			$options,
		];
	}
