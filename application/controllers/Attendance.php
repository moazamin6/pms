...
class Attendance extends MY_Controller
{
	...
	function save()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_row_data($save_id), 'id' => $save_id, 'isUpdate' => $id ? true : false, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	function save_note()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_row_data($save_id), 'id' => $save_id, 'isUpdate' => true, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function log_time($user_id = 0)
	{
		...
		if($user_id)
		{
			echo json_encode(["success" => true, "data" => $this->_clock_in_out_row_data($user_id), 'id' => $user_id, 'message' => lang('record_saved'), "isUpdate" => true]);
		}
		else
		{
			if($this->input->post("clock_out"))
			{
				echo json_encode(["success" => true, "clock_widget" => clock_widget(true)]);
			}
			else
			{
				clock_widget();
	...
	function list_data()
	{
		...
		$options = ["start_date" => $start_date, "end_date" => $end_date, "login_user_id" => $this->login_user->id, "user_id" => $user_id, "access_type" => $this->access_type, "allowed_members" => $this->allowed_members];
	...
	private function _make_row($data)
	{
		...
		$option_links = modal_anchor(get_uri("attendance/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_attendance'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_attendance'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("attendance/delete"), "data-action" => "delete"]);
		...
		$note_link = modal_anchor(get_uri("attendance/note_modal_form"), "<i class='fa fa-comment-o p10'></i>", ["class" => "edit text-muted", "title" => lang("note"), "data-post-id" => $data->id]);
		if($data->note)
		{
			$note_link = modal_anchor(get_uri("attendance/note_modal_form"), "<i class='fa fa-comment p10'></i>", ["class" => "edit text-muted", "title" => $note_title, "data-modal-title" => lang("note"), "data-post-id" => $data->id]);
		}
	...
	private function _get_members_query_options($type = "")
	{
		...
		else
		{
			...
			else
			{
				...
				$where = ["user_type" => "staff", "where_in" => ($type == "data") ? $allowed_members : ["id" => $allowed_members]];
			}
		}
	...
	function clocked_in_members_list_data()
	{
		...
		$options = ["login_user_id" => $this->login_user->id, "access_type" => $this->access_type, "allowed_members" => $this->allowed_members, "only_clocked_in_members" => true];
		$list_data = $this->Attendance_model->get_details($options)->result();
	...
	private function _make_clock_in_out_row($data)
	{
		if(isset($data->attendance_id))
		{
			...
			$view_data = modal_anchor(get_uri("attendance/note_modal_form/$data->id"), "<i class='fa fa-sign-out'></i> " . lang('clock_out'), ["class" => "btn btn-default", "title" => lang('clock_out'), "id" => "timecard-clock-out", "data-post-id" => $data->attendance_id, "data-post-clock_out" => 1, "data-post-id" => $data->id]);
		}
		else
		{
			$status = "<div class='mb15'>" . lang('not_clocked_id_yet') . "</div>";
			$view_data = js_anchor("<i class='fa fa-sign-in'></i> " . lang('clock_in'), ['title' => lang('clock_in'), "class" => "btn btn-default", "data-action-url" => get_uri("attendance/log_time/$data->id"), "data-action" => "update", "data-inline-loader" => "1", "data-post-id" => $data->id]);
		}
