...
class Team_members extends MY_Controller
{
	...
	private function can_update_team_members_info($user_id)
	{
		...
		else
		{
			...
			{
				if($user_id && in_array($user_id, $access_info->allowed_members))
				{
	...
	function add_team_member()
	{
		...
		if($user_id)
		{
			echo json_encode(["success" => true, "data" => $this->_row_data($user_id), 'id' => $user_id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	function send_invitation()
	{
		...
		if(!in_array(false, $send_email))
		{
			...
			{
				echo json_encode(['success' => true, 'message' => lang("invitations_sent")]);
			}
		}
		else
		{
	...
	private function _make_row($data, $custom_fields)
	{
		...
		{
			$delete_link = js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_team_member'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("team_members/delete"), "data-action" => "delete-confirmation"]);
		}
	...
	function view($id = 0, $tab = "")
	{
		...
		else
		{
			...
			$view_data['team_members'] = $this->Users_model->get_details(["user_type" => "staff", "status" => "active"])->result();
	...
	{
		//setting preferences
		$settings = ["notification_sound_volume", "disable_push_notification", "hidden_topbar_menus", "disable_keyboard_shortcuts", "recently_meaning"];
	...
	private function _make_file_row($data)
	{
		...
		$description = "<div class='pull-left'>" .
			js_anchor(remove_file_prefix($data->file_name), ['title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("team_members/view_file/" . $data->id)]);
		...
		$options = anchor(get_uri("team_members/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", ["title" => lang("download")]);
		
		if($this->login_user->is_admin || $data->uploaded_by == $this->login_user->id)
		{
			$options .= js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("team_members/delete_file"), "data-action" => "delete-confirmation"]);
		}
	...
	function delete_file()
	{
		...
		if($info->user_id && ($this->login_user->is_admin || $this->login_user->id === $info->uploaded_by))
		{
			if($this->General_files_model->delete($id))
			{
				...
				//delete the files
				delete_app_files(get_general_file_path("team_members", $info->user_id), [make_array_of_file($info)]);
