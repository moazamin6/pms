...
class Estimates extends MY_Controller
{
	...
	function save()
	{
		...
		if($estimate_id)
		{
			...
			if($is_clone && $main_estimate_id)
			{
				...
				$estimate_items = $this->Estimate_items_model->get_all_where(["estimate_id" => $main_estimate_id, "deleted" => 0])->result();
			...
			{
				$order_items = $this->Order_items_model->get_details(["order_id" => $copy_items_from_order])->result();
			...
			echo json_encode(["success" => true, "data" => $this->_row_data($estimate_id), 'id' => $estimate_id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	private function _create_project_from_estimate($estimate_id)
	{
		if($estimate_id)
		{
			...
			if(!$this->Projects_model->get_one_where(["estimate_id" => $estimate_id])->id)
			{
				$data = [
	...
	function estimate_list_data_of_client($client_id)
	{
		...
		$options = ["client_id" => $client_id, "status" => $this->input->post("status"), "custom_fields" => $custom_fields];
	...
	private function _make_row($data, $custom_fields)
	{
		...
		$row_data[] = modal_anchor(get_uri("estimates/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_estimate'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_estimate'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("estimates/delete"), "data-action" => "delete"]);
	...
	private function _get_estimate_status_label($estimate_info, $return_html = true)
	{
		...
		if($this->login_user->user_type == "client")
		{
			...
			{
				if($estimate_info->status == "declined")
				{
		...
		{
			if($estimate_info->status == "declined" || $estimate_info->status == "rejected")
			{
	...
	function save_discount()
	{
		...
		if($save_data)
		{
			echo json_encode(["success" => true, "estimate_total_view" => $this->_get_estimate_total_view($estimate_id), 'message' => lang('record_saved'), "estimate_id" => $estimate_id]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function save_item()
	{
		...
		if($estimate_item_id)
		{
			...
			echo json_encode(["success" => true, "estimate_id" => $item_info->estimate_id, "data" => $this->_make_item_row($item_info), "estimate_total_view" => $this->_get_estimate_total_view($item_info->estimate_id), 'id' => $estimate_item_id, 'message' => lang('record_saved')]);
		}
		else
		{
			echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
	...
	function delete_item()
	{
		...
		if($this->input->post('undo'))
		{
			if($this->Estimate_items_model->delete($id, true))
			{
				$options = ["id" => $id];
				$item_info = $this->Estimate_items_model->get_details($options)->row();
				echo json_encode(["success" => true, "estimate_id" => $item_info->estimate_id, "data" => $this->_make_item_row($item_info), "estimate_total_view" => $this->_get_estimate_total_view($item_info->estimate_id), "message" => lang('record_undone')]);
			}
			else
			{
				echo json_encode(["success" => false, lang('error_occurred')]);
		...
		else
		{
			if($this->Estimate_items_model->delete($id))
			{
				...
				echo json_encode(["success" => true, "estimate_id" => $item_info->estimate_id, "estimate_total_view" => $this->_get_estimate_total_view($item_info->estimate_id), 'message' => lang('record_deleted')]);
			}
			else
			{
				echo json_encode(["success" => false, 'message' => lang('record_cannot_be_deleted')]);
	...
	function item_list_data($estimate_id = 0)
	{
		...
		$list_data = $this->Estimate_items_model->get_details(["estimate_id" => $estimate_id])->result();
		$result = [];
		foreach($list_data as $data)
		{
	...
	private function _make_item_row($data)
	{
		...
		return [
			...
			modal_anchor(get_uri("estimates/item_modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_estimate'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("estimates/delete_item"), "data-action" => "delete"]),
		];
	}
	...
	function get_estimate_status_bar($estimate_id = 0)
	{
		...
		$view_data["estimate_info"] = $this->Estimates_model->get_details(["id" => $estimate_id])->row();
		$view_data['estimate_status_label'] = $this->_get_estimate_status_label($view_data["estimate_info"]);
	...
	function send_estimate_modal_form($estimate_id)
	{
		...
		if($estimate_id)
		{
			...
			if($is_lead)
			{
				$contacts_options = ["user_type" => "lead", "client_id" => $estimate_info->client_id];
			}
			else
			{
				$contacts_options = ["user_type" => "client", "client_id" => $estimate_info->client_id];
			}
	...
	function send_estimate()
	{
		...
		if(send_app_mail($contact->email, $subject, $message, ["attachments" => [["file_path" => $attachement_url]], "cc" => $cc, "bcc" => $bcc_emails]))
		{
			// change email status
			$status_data = ["status" => "sent", "last_email_sent_date" => get_my_local_time()];
			if($this->Estimates_model->save($status_data, $estimate_id))
			{
				echo json_encode(['success' => true, 'message' => lang("estimate_sent_message"), "estimate_id" => $estimate_id]);
			}
