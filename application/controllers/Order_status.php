...
class Order_status extends MY_Controller
{
	...
	function index()
	{
		$this->load->view("order_status/index");
	}
	...
	function modal_form()
	{
		...
		$view_data['model_info'] = $this->Order_status_model->get_one($this->input->post('id'));
		$this->load->view('order_status/modal_form', $view_data);
	}
	...
	function save()
	{
		...
		$save_id = $this->Order_status_model->save($data, $id);
	...
	function update_field_sort_values($id = 0)
	{
		...
		if($sort_values)
		{
			...
			foreach($sort_array as $value)
			{
				...
				$data = ["sort" => $sort];
				$this->Order_status_model->save($data, $id);
			}
		}
	}
	...
	function list_data()
	{
		$list_data = $this->Order_status_model->get_details()->result();
		$result = [];
	...
	{
		$options = ["id" => $id];
		$data = $this->Order_status_model->get_details($options)->row();
		return $this->_make_row($data);
	}

	private function _make_row($data)
	{
		$edit = modal_anchor(get_uri("order_status/modal_form"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_order_status'), "data-post-id" => $data->id]);

		$delete_attributes = ['title' => lang('delete_order_status'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("order_status/delete"), "data-action" => "delete-confirmation"];
		if($data->total_orders)
		{
			//the status shouldn't be deleted if there is any orders
			$delete_attributes = ['title' => lang('there_has_orders_with_this_status'), "class" => "delete not-clickable text-off",];
		}
