...
class Custom_fields extends MY_Controller
{
	...
	function save()
	{
		...
		if($save_id)
		{
			echo json_encode(["success" => true, "data" => $this->_row_data($save_id), 'newData' => $id ? false : true, 'id' => $save_id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	private function _make_field_row($data)
	{
		...
		return [
			$field,
			$data->sort,
			modal_anchor(get_uri("custom_fields/modal_form/"), "<i class='fa fa-pencil'></i>", ["class" => "edit", "title" => lang('edit_field'), "data-post-id" => $data->id])
			. js_anchor("<i class='fa fa-times fa-fw'></i>", ['title' => lang('delete_field'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("custom_fields/delete"), "data-action" => "delete"]),
		];
	}
