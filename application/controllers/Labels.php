...
class Labels extends MY_Controller
{
	...
	private function can_access_labels_of_this_context($context = "", $label_id = 0)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if($context == "event" || $context == "note" || $context == "to_do")
					{
						if($label_id)
						{
	...
	function save()
	{
		...
		if($save_id)
		{
			...
			echo json_encode(["success" => true, 'data' => $this->_get_labels_row_data($label_info), 'id' => $id, 'message' => lang('record_saved')]);
		}
		else
		{
	...
	function delete()
	{
		...
		if($existing_labels)
		{
			echo json_encode(["label_exists" => true, 'message' => lang("label_existing_error_message")]);
		}
		else
		{
			if($this->Labels_model->delete($id))
			{
				echo json_encode(["success" => true, 'id' => $id, 'message' => lang('record_saved')]);
			}
			else
			{
				echo json_encode(["success" => false, 'message' => lang('error_occurred')]);
