<div id="kanban-wrapper">
	...
	foreach($leads as $lead)
	{
		...
		$open_in_new_tab = anchor(get_uri("leads/view/" . $lead->id), "<i class='fa fa-external-link'></i>", ["target" => "_blank", "class" => "invisible pull-right text-off", "title" => lang("details")]);

		$make_client = modal_anchor(get_uri("leads/make_client_modal_form/") . $lead->id, "<i class='fa fa-briefcase'></i>", ["title" => lang('make_client'), "class" => "pull-right mr5 invisible text-off"]);
		...
		if($kanban_custom_fields)
		{
			...
			{
				$kanban_custom_fields_data .= "<br /><small>" . get_array_value($kanban_custom_field, "custom_field_title") . ": " . $this->load->view("custom_fields/output_" . get_array_value($kanban_custom_field, "custom_field_type"), ["value" => get_array_value($kanban_custom_field, "value")], true) . "</small>";
			}
		}
