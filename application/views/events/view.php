<div class="modal-body event-view">
   <div class="table-responsive mb15">
		  ...
		  <?php if($model_info->google_event_id){ ?>
             <div class="pull-right pb10 ">
                <i class="fa fa-external-link"></i>
				 <?php echo anchor(get_uri("events/show_event_in_google_calendar/$model_info->google_event_id"), lang("open_in_google_calendar"), ["target" => "_blank"]); ?>
             </div>
		  <?php } ?>
	   ...
	   <?php if($model_info->description){ ?>
          <div class="col-md-12">
             <blockquote class="font-14 text-justify"
                         style="<?php echo "border-color:" . $model_info->color; ?>"><?php echo nl2br($model_info->description); ?></blockquote>
          </div>
	   <?php } ?>
      ...
      <div class="col-md-12 pt10 pb10">
		  ...
		  echo "<span class='avatar avatar-xs mr10'><img src='$image_url' alt=''></span><span>" . get_team_member_profile_link($model_info->created_by, $model_info->created_by_name, ["class" => "dark strong"]) . "</span>";
		  ?>
      </div>
...
<div class="modal-footer">
	...
	if(isset($editable) && $editable === "1")
	{
		...
		if($this->login_user->id == $model_info->created_by || $this->login_user->is_admin)
		{
			...
			if($show_delete)
			{
				echo js_anchor("<i class='fa fa-times-circle-o'></i> " . lang('delete_event'), ["class" => "btn btn-default pull-left", "id" => "delete_event", "data-encrypted_event_id" => $encrypted_event_id]);
			}

			echo modal_anchor(get_uri("events/modal_form/"), "<i class='fa fa-pencil'></i> " . lang('edit_event'), ["class" => "btn btn-default", "data-post-encrypted_event_id" => $encrypted_event_id, "title" => lang('edit_event')]);
		}
	}
