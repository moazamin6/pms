        ...
        <span class="pull-right dropdown dashboard-dropdown ml10">
            ...
            <ul class="dropdown-menu" role="menu">
                <?php if($dashboard_type == "default"){ ?>
                   <li role="presentation"><?php echo modal_anchor(get_uri("dashboard/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_new_dashboard'), ["title" => lang('add_new_dashboard')]); ?> </li>
				<?php } else{ ?>
                   <li role="presentation"
                       class="hidden-xs"><?php echo anchor(get_uri("dashboard/edit_dashboard/" . $dashboard_info->id), "<i class='fa fa-columns'></i> " . lang('edit_dashboard'), ["title" => lang('edit_dashboard')]); ?> </li>
                   <li role="presentation"><?php echo modal_anchor(get_uri("dashboard/modal_form/" . $dashboard_info->id), "<i class='fa fa-pencil'></i> " . lang('edit_title'), ["title" => lang('edit_title'), "id" => "dashboard-edit-title-button"]); ?> </li>
                   <li role="presentation"><?php echo js_anchor("<i class='fa fa-times fa-fw'></i>" . lang('delete'), ['title' => lang('delete'), "class" => "delete", "data-id" => $dashboard_info->id, "data-action-url" => get_uri("dashboard/delete"), "data-action" => "delete-confirmation", "data-success-callback" => "onDashboardDeleteSuccess"]); ?> </li>
				<?php } ?>
            </ul>
        </span>
