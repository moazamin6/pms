<div class="list-group" id="js-dashboard-popup-list">
	<?php
	//default dashboard
	echo anchor(get_uri("dashboard"), "<i class='fa fa-desktop mr10'></i> " . lang("default_dashboard"), ["class" => "list-group-item"]);
	...
	if(!(get_setting("disable_dashboard_customization_by_clients") && $this->login_user->user_type == "client"))
	{
		if(count($dashboards))
		{
			...
			foreach($dashboards as $dashboard)
			{
				...
				echo anchor(get_uri("dashboard/view/" . $dashboard->id), $title, ["class" => "list-group-item js-dashboard-list", "data-value" => $dashboard->id, "data-id" => $dashboard->id, "data-sort" => $dashboard->new_sort]);
			}

			echo "</div>";
		}

		echo modal_anchor(get_uri("dashboard/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_new_dashboard'), ["class" => "list-group-item text-center", "title" => lang('add_new_dashboard')]);
	}
	?>
</div>
