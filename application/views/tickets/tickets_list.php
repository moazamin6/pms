<div id="page-content" class="p20 clearfix">
   ...
   <ul class="nav nav-tabs bg-white title" role="tablist">
      <li class="title-tab"><h4 class="pl15 pt10 pr15"><?php echo lang('tickets'); ?></h4></li>
	   ...
	   <?php $this->load->view("tickets/index", ["active_tab" => "tickets_list"]); ?>

      <div class="tab-title clearfix no-border">
         <div class="title-button-group">
			 <?php echo modal_anchor(get_uri("labels/modal_form"), "<i class='fa fa-tags'></i> " . lang('manage_labels'), ["class" => "btn btn-default", "title" => lang('manage_labels'), "data-post-type" => "ticket"]); ?>
			 <?php echo modal_anchor(get_uri("tickets/settings_modal_form"), "<i class='fa fa fa-cog'></i> " . lang('settings'), ["class" => "btn btn-default", "title" => lang('settings')]); ?>
			 <?php echo modal_anchor(get_uri("tickets/modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_ticket'), ["class" => "btn btn-default", "title" => lang('add_ticket')]); ?>
         </div>
      </div>
...
<script type="text/javascript">
   $(document).ready(function () {
      ...
      $("#ticket-table").appTable({
         ...
         order: [[6, "desc"]],
         radioButtons: [{text: '<?php echo lang("open") ?>', name: "status", value: "open", isChecked: selectOpenStatus}, {
            text: '<?php echo lang("closed") ?>',
            name: "status",
            value: "closed",
            isChecked: selectClosedStatus
         }],
         filterDropdown: [{name: "ticket_type_id", class: "w200", options: <?php echo $ticket_types_dropdown; ?>}, {
            name: "ticket_label",
            class: "w200",
            options: <?php echo $ticket_labels_dropdown; ?>}, {name: "assigned_to", class: "w200", options: <?php echo $assigned_to_dropdown; ?>}],
         singleDatepicker: [{
