   ...
   <span class="text-off ml15"><?php echo lang("created") . ": "; ?></span>
	...
	<?php if($ticket_info->task_id != "0"){ ?>
       <span class="text-off ml15"><?php echo lang("task") . ": "; ?></span>
		<?php echo modal_anchor(get_uri("projects/task_view"), $ticket_info->task_title, ["title" => lang('task_info') . " #$ticket_info->task_id", "data-post-id" => $ticket_info->task_id, "data-modal-lg" => "1"]) ?>
	<?php } ?>
</div>
...
if(count($custom_fields_list))
{
	...
	foreach($custom_fields_list as $data)
	{
		if($data->value)
		{
			$fields .= "<div class='p15 bg-white b-b '><i class='fa fa-check-square ml15'></i> <span class='text-off'> $data->title:</span> " . $this->load->view("custom_fields/output_" . $data->field_type, ["value" => $data->value], true) . "</div>";
		}
	}
	if($fields)
	{
