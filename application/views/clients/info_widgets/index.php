	...
	<?php if($show_invoice_info){ ?>
		<?php if(!in_array("projects", $hidden_menu)){ ?>
          <div class="col-md-3 col-sm-6 widget-container">
			  <?php $this->load->view("clients/info_widgets/tab", ["tab" => "projects"]); ?>
          </div>
		<?php } ?>

		<?php if(!in_array("invoices", $hidden_menu)){ ?>
          <div class="col-md-3 col-sm-6  widget-container">
			  <?php $this->load->view("clients/info_widgets/tab", ["tab" => "invoice_value"]); ?>
          </div>
		<?php } ?>

		<?php if(!in_array("payments", $hidden_menu) && !in_array("invoices", $hidden_menu)){ ?>
          <div class="col-md-3 col-sm-6  widget-container">
			  <?php $this->load->view("clients/info_widgets/tab", ["tab" => "payments"]); ?>
          </div>
          <div class="col-md-3 col-sm-6  widget-container">
			  <?php $this->load->view("clients/info_widgets/tab", ["tab" => "due"]); ?>
          </div>
		<?php } ?>
