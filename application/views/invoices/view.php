                ...
                <span class="dropdown inline-block mt10">
                    ...
                    <ul class="dropdown-menu" role="menu">
                        <?php if($invoice_status !== "cancelled" && $can_edit_invoices){ ?>
                           <li role="presentation"><?php echo modal_anchor(get_uri("invoices/send_invoice_modal_form/" . $invoice_info->id), "<i class='fa fa-envelope-o'></i> " . lang('email_invoice_to_client'), ["title" => lang('email_invoice_to_client'), "data-post-id" => $invoice_info->id, "role" => "menuitem", "tabindex" => "-1"]); ?> </li>
						<?php } ?>
                        <li role="presentation"><?php echo anchor(get_uri("invoices/download_pdf/" . $invoice_info->id), "<i class='fa fa-download'></i> " . lang('download_pdf'), ["title" => lang('download_pdf')]); ?> </li>
                        <li role="presentation"><?php echo anchor(get_uri("invoices/download_pdf/" . $invoice_info->id . "/view"), "<i class='fa fa-file-pdf-o'></i> " . lang('view_pdf'), ["title" => lang('view_pdf'), "target" => "_blank"]); ?> </li>
                        <li role="presentation"><?php echo anchor(get_uri("invoices/preview/" . $invoice_info->id . "/1"), "<i class='fa fa-search'></i> " . lang('invoice_preview'), ["title" => lang('invoice_preview'), "target" => "_blank"]); ?> </li>
                        <li role="presentation"><?php echo js_anchor("<i class='fa fa-print'></i> " . lang('print_invoice'), ['title' => lang('print_invoice'), 'id' => 'print-invoice-btn']); ?> </li>

                        <?php if($can_edit_invoices){ ?>
                           <li role="presentation" class="divider"></li>

                            <?php if($invoice_status !== "cancelled"){ ?>
                              <li role="presentation"><?php echo modal_anchor(get_uri("invoices/modal_form"), "<i class='fa fa-edit'></i> " . lang('edit_invoice'), ["title" => lang('edit_invoice'), "data-post-id" => $invoice_info->id, "role" => "menuitem", "tabindex" => "-1"]); ?> </li>
							<?php } ?>

                            <?php if($invoice_status == "draft" && $invoice_status !== "cancelled"){ ?>
                              <li role="presentation"><?php echo ajax_anchor(get_uri("invoices/update_invoice_status/" . $invoice_info->id . "/not_paid"), "<i class='fa fa-check'></i> " . lang('mark_invoice_as_not_paid'), ["data-reload-on-success" => "1"]); ?> </li>
							<?php } else{
								if($invoice_status == "not_paid" || $invoice_status == "overdue" || $invoice_status == "partially_paid"){ ?>
                                   <li role="presentation"><?php echo js_anchor("<i class='fa fa-close'></i> " . lang('mark_invoice_as_cancelled'), ['title' => lang('mark_invoice_as_cancelled'), "data-action-url" => get_uri("invoices/update_invoice_status/" . $invoice_info->id . "/cancelled"), "data-action" => "delete-confirmation", "data-reload-on-success" => "1"]); ?> </li>
								<?php }
							} ?>

                           <li role="presentation"><?php echo modal_anchor(get_uri("invoices/modal_form"), "<i class='fa fa-copy'></i> " . lang('clone_invoice'), ["data-post-is_clone" => true, "data-post-id" => $invoice_info->id, "title" => lang('clone_invoice')]); ?></li>
						<?php } ?>

                    </ul>
                </span>
			 <?php if($invoice_status !== "cancelled" && $can_edit_invoices){ ?>
				 <?php echo modal_anchor(get_uri("invoices/item_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_item'), ["class" => "btn btn-default", "title" => lang('add_item'), "data-post-invoice_id" => $invoice_info->id]); ?>
				 <?php echo modal_anchor(get_uri("invoice_payments/payment_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_payment'), ["class" => "btn btn-default", "title" => lang('add_payment'), "data-post-invoice_id" => $invoice_info->id]); ?>
			 <?php } ?>
         </div>
      </div>
      ...
      <div class="mt15">
         <div class="panel panel-default p15 b-t">
            ...
            <div class="clearfix">
               <div class="pull-right pr15" id="invoice-total-section">
				   <?php $this->load->view("invoices/invoice_total_section", ["invoice_id" => $invoice_info->id, "can_edit_invoices" => $can_edit_invoices]); ?>
               </div>
            </div>
                   ...
                   <div class="col-md-12 mt20">
                      ...
                      <div class="mb5 strong"><?php echo lang("files"); ?></div>
					   ...
					   foreach($files as $key => $value)
					   {
						   ...
						   echo "<div>";
						   echo js_anchor(remove_file_prefix($file_name), ["data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("invoices/file_preview/" . $invoice_info->id . "/" . $key)]);
						   echo "</div>";
					   }
					   ?>
                   </div>
	   ...
	   <?php if($invoice_info->recurring){ ?>
          <ul id="invoice-view-tabs" data-toggle="ajax-tab" class="nav nav-tabs" role="tablist">
             ...
             <li><a role="presentation" href="<?php echo_uri("invoices/sub_invoices/" . $invoice_info->id); ?>"
                    data-target="#sub-invoices"> <?php echo lang('sub_invoices'); ?></a></li>
          </ul>
