<span class="invoice-info-title"
      style="font-size:20px; font-weight: bold;background-color: <?php echo $color; ?>; color: #fff;">&nbsp;<?php echo get_invoice_id($invoice_info->id); ?>&nbsp;</span>
...
if(isset($invoice_info->custom_fields) && $invoice_info->custom_fields)
{
	foreach($invoice_info->custom_fields as $field)
	{
		if($field->value)
		{
			echo "<span>" . $field->custom_field_title . ": " . $this->load->view("custom_fields/output_" . $field->custom_field_type, ["value" => $field->value], true) . "</span><br />";
		}
	}
}
?>
