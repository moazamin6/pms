...
if(!function_exists("make_project_tabs_data"))
{
	...
	function make_project_tabs_data($default_project_tabs = [], $is_client = false)
	{
		...
		{
			if(!$is_client && $project_tab_order)
			{
				//user is team member
            ...
            <div class="page-title clearfix">
               <h1>
				   ...
				   <?php } else{
					   ...
					   <?php } else{
						   if($project_info->status == "hold"){ ?>
                              <i class="fa fa-pause" title="<?php echo lang("hold"); ?>"></i>
						   <?php } else{
							   if($project_info->status == "canceled"){ ?>
				   ...
				   <?php if(!(get_setting("disable_access_favorite_project_option_for_clients") && $this->login_user->user_type == "client")){ ?>
                      <span id="star-mark">
							...
							if($is_starred)
							{
								$this->load->view('projects/star/starred', ["project_id" => $project_info->id]);
							}
							else
							{
								$this->load->view('projects/star/not_starred', ["project_id" => $project_info->id]);
							}
							?>
                        </span>
				   <?php } ?>
               ...
               <div class="title-button-group" id="project-timer-box">
				   ...
				   {
					   echo modal_anchor(get_uri("projects/settings_modal_form"), "<i class='fa fa fa-cog'></i> " . lang('settings'), ["class" => "btn btn-default", "title" => lang('settings'), "data-post-project_id" => $project_info->id]);
				   }
				   ?>
				   <?php if($show_actions_dropdown){ ?>
                      <span class="dropdown inline-block">
                            ...
                            <ul class="dropdown-menu dropdown-left" role="menu">
                                <?php if($project_info->status == "open"){ ?>
                                   <li role="presentation"><?php echo ajax_anchor(get_uri("projects/change_status/$project_info->id/completed"), "<i class='fa fa-check-circle'></i> " . lang('mark_project_as_completed'), ["class" => "", "title" => lang('mark_project_as_completed'), "data-reload-on-success" => "1"]); ?> </li>
                                   <li role="presentation"><?php echo ajax_anchor(get_uri("projects/change_status/$project_info->id/hold"), "<i class='fa fa-pause'></i> " . lang('mark_project_as_hold'), ["class" => "", "title" => lang('mark_project_as_hold'), "data-reload-on-success" => "1"]); ?> </li>
                                   <li role="presentation"><?php echo ajax_anchor(get_uri("projects/change_status/$project_info->id/canceled"), "<i class='fa fa-times'></i> " . lang('mark_project_as_canceled'), ["class" => "", "title" => lang('mark_project_as_canceled'), "data-reload-on-success" => "1"]); ?> </li>
								<?php } ?>
								<?php if($project_info->status != "open"){ ?>
                                   <li role="presentation"><?php echo ajax_anchor(get_uri("projects/change_status/$project_info->id/open"), "<i class='fa fa-th-large'></i> " . lang('mark_project_as_open'), ["class" => "", "title" => lang('mark_project_as_open'), "data-reload-on-success" => "1"]); ?> </li>
									<?php
								}
								if($this->login_user->user_type == "staff")
								{
									echo "<li role='presentation'>" . modal_anchor(get_uri("projects/clone_project_modal_form"), "<i class='fa fa-copy'></i> " . lang('clone_project'), ["class" => "", "data-post-id" => $project_info->id, "title" => lang('clone_project')]) . " </li>";
								}
								echo "<li role='presentation'>" . modal_anchor(get_uri("projects/modal_form"), "<i class='fa fa-pencil'></i> " . lang('edit_project'), ["class" => "edit", "title" => lang('edit_project'), "data-post-id" => $project_info->id]) . " </li>";
								?>
...
if($preview_task_id)
{
	echo modal_anchor(get_uri("projects/task_view"), "", ["id" => "preview_task_link", "title" => lang('task_info') . " #$preview_task_id", "data-post-id" => $preview_task_id, "data-modal-lg" => "1"]);
}
?>
