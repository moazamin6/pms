<div class="panel">
   <div class="tab-title clearfix">
      ...
      <div class="title-button-group">
		  ...
		  {
			  echo modal_anchor(get_uri("labels/modal_form"), "<i class='fa fa-tags'></i> " . lang('manage_labels'), ["class" => "btn btn-default", "title" => lang('manage_labels'), "data-post-type" => "task"]);
			  echo modal_anchor("", "<i class='fa fa-pencil-square'></i> " . lang('batch_update'), ["class" => "btn btn-info hide batch-update-btn", "title" => lang('batch_update'), "data-post-project_id" => $project_id]);
			  echo js_anchor("<i class='fa fa-check-square'></i> " . lang("batch_update"), ["class" => "btn btn-default batch-active-btn"]);
			  echo js_anchor("<i class='fa fa-close'></i> " . lang("cancel_selection"), ["class" => "hide btn btn-default batch-cancel-btn"]);
		  }
		  if($can_create_tasks)
		  {
			  echo modal_anchor(get_uri("projects/task_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_multiple_tasks'), ["class" => "btn btn-default", "title" => lang('add_multiple_tasks'), "data-post-project_id" => $project_id, "data-post-add_type" => "multiple"]);
			  echo modal_anchor(get_uri("projects/task_modal_form"), "<i class='fa fa-plus-circle'></i> " . lang('add_task'), ["class" => "btn btn-default", "title" => lang('add_task'), "data-post-project_id" => $project_id]);
		  }
		  ?>
      </div>
   </div>
...
foreach($task_statuses as $status)
{
	...
	$statuses[] = ["text" => ($status->key_name ? lang($status->key_name) : $status->title), "value" => $status->id, "isChecked" => $is_selected];
}
?>
