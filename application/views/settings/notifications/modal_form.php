<?php echo form_open(get_uri("settings/save_notification_settings"), ["id" => "notification-settings-form", "class" => "general-form", "role" => "form"]); ?>
<div class="modal-body clearfix">
   ...
   <div class="form-group">
	...
	<?php if($model_info->event !== "new_message_sent" && $model_info->event !== "message_reply_sent"){ ?>
       <div class="form-group">
          <label for="enable_slack" class="col-md-3"><?php echo lang('enable_slack'); ?></label>
          <div class="col-md-9">
			  <?php
			  echo form_checkbox("enable_slack", "1", $model_info->enable_slack ? true : false, "id='enable_slack'");
			  ?>
          </div>
       </div>
	<?php } ?>
		  ...
		  foreach($notify_to as $notify_optoin)
		  {
			  if($notify_optoin === "team")
			  {
                 ...
                 <div class="pb10">
                    ...
                    <div>
                       <input type="text" value="<?php echo $model_info->notify_to_team; ?>" name="team" id="team_dropdown" class="w100p"
                              placeholder="<?php echo lang('team'); ?>"/>
                    </div>
                 </div>
			  <?php } else{
				  if($notify_optoin === "team_members"){ ?>
                     <div class="pb10">
                        <label for="notify_to_team_members"><?php echo lang('team_members'); ?></label>
                        <div>
                           <input type="text" value="<?php echo $model_info->notify_to_team_members; ?>" name="team_members" id="team_members_dropdown"
                                  class="w100p" placeholder="<?php echo lang('team_members'); ?>"/>
                        </div>
                     </div>
				  <?php }
				  else
