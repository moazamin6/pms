                   ...
                   <div class="media clearfix">
                      ...
                      <div class="media-body">
                         <div class="clearfix mb15">
                            ...
                            <div class="media-left ">
                               <div class="mt5"><?php echo get_team_member_profile_link($post->created_by, $post->created_by_user, ["class" => "dark strong"]); ?></div>
							 ...
							 <?php if($this->login_user->is_admin || $post->created_by == $this->login_user->id){ ?>
                                <span class="pull-right mt-50 dropdown">
                                            ...
                                            <ul class="dropdown-menu" role="menu">
                                                <li role="presentation"><?php echo ajax_anchor(get_uri("timeline/delete/$post->id"), "<i class='fa fa-times'></i> " . lang('delete'), ["class" => "", "title" => lang('delete'), "data-fade-out-on-success" => "#post-content-container-$post->id"]); ?> </li>
                                            </ul>
                                        </span>
							 <?php } ?>
                         ...
                         <div class="mb15 clearfix">
							 <?php
							 echo ajax_anchor(get_uri("timeline/post_reply_form/" . $post->id), "<i class='fa fa-reply font-11'></i> " . lang('reply'), ["data-real-target" => "#reply-form-container-" . $post->id, "class" => "dark"]);
							 ?>
							 <?php
							 $reply_caption = "";
							 ...
							 if($reply_caption)
							 {
								 echo ajax_anchor(get_uri("timeline/view_post_replies/" . $post->id), "<i class='fa fa-comment-o'></i> " . lang("view") . " " . $post->total_replies . " " . $reply_caption, ["class" => "btn btn-default btn-xs view-replies", "id" => "show-replies-button-$post->id", "data-remove-on-success" => "#show-replies-button-$post->id", "data-real-target" => "#reply-list-" . $post->id]);
							 }
							 //create link for reply success. trigger this link after submit any reply
							 echo ajax_anchor(get_uri("timeline/view_post_replies/" . $post->id), "", ["class" => "hide", "id" => "reload-reply-list-button-" . $post->id, "data-real-target" => "#reply-list-" . $post->id]);
							 ...
							 if($total_files)
							 {
								 ...
								 echo anchor(get_uri("timeline/download_files/" . $post->id), $download_caption, ["class" => "pull-right", "title" => $download_caption]);
							 }
							 ?>
...
if($result_remaining > 0)
{
   ...
   <div id="loader-<?php echo $next_container_id; ?>">
      <div class="text-center ml30">
		  <?php
		  echo ajax_anchor(get_uri("timeline/load_more_posts/" . $next_page_offset), lang("load_more"), ["class" => "btn btn-default load-more mt15 p10", "data-remove-on-success" => "#loader-" . $next_container_id, "title" => lang("load_more"), "data-inline-loader" => "1", "data-real-target" => "#" . $next_container_id]);
		  ?>
      </div>
   </div>
	<?php
