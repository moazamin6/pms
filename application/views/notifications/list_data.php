...
if(count($notifications))
{
	...
	foreach($notifications as $notification)
	{
		...
		if($notification->user_id)
		{
			...
			{
				if($notification->user_id == "999999997")
				{
		...
		if($notification->activity_log_changes == "" || ($notification->activity_log_changes !== "" && count($changes_array)))
		{
           ...
           <a class="list-group-item <?php echo $notification_class; ?>"
              data-notification-id="<?php echo $notification->id; ?>" <?php echo $url_attributes; ?> >
              ...
              <div class="media-body w100p">
                 ...
                 <div class="media m0">
					 ...
					 $this->load->view("notifications/notification_description", ["notification" => $notification, "changes_array" => $changes_array]);
					 ?>
                 </div>
              </div>
           </a>
	...
	if($result_remaining)
	{
       ...
       <div id="loader-<?php echo $next_container_id; ?>">
          <div class="text-center p20 clearfix mt-5">
			  <?php
			  echo ajax_anchor(get_uri("notifications/load_more/" . $next_page_offset), lang("load_more"), ["class" => "btn btn-default load-more mt15 p10", "data-remove-on-success" => "#loader-" . $next_container_id, "title" => lang("load_more"), "data-inline-loader" => "1", "data-real-target" => "#" . $next_container_id]);
			  ?>
          </div>
       </div>
		<?php
