<div class="box" id="profile-image-section">
   <div class="box-content w200 text-center profile-image">
	   ...
	   {
		   if($user_info->user_type === "lead")
		   {
			   $url = "leads";
		   }
	   }
	   echo form_open(get_uri($url . "/save_profile_image/" . $user_info->id), ["id" => "profile-image-form", "class" => "general-form", "role" => "form"]);
	   ?>
	   <?php if($this->login_user->is_admin || $user_info->id === $this->login_user->id){ ?>
          <div class="file-upload btn mt0 p0 profile-image-upload" data-toggle="tooltip" title="<?php echo lang("upload_and_crop"); ?>" data-placement="right">
             ...
             <input id="profile_image_file" class="upload" name="profile_image_file" type="file" data-height="200" data-width="200"
                    data-preview-container="#profile-image-preview" data-input-field="#profile_image"/>
          </div>
          <div class="file-upload btn p0 profile-image-upload profile-image-direct-upload" data-toggle="tooltip"
               title="<?php echo lang("upload"); ?> (200x200 px)" data-placement="right">
			  <?php
			  echo form_upload([
      ...
      <div class="p10 m0 clearfix">
         <div class="pull-left">
		  ...
		  if($user_info->id != $this->login_user->id)
		  {
			  ...
			  {
				  echo modal_anchor(get_uri("messages/modal_form/" . $user_info->id), "<i class='fa fa-envelope-o'></i> " . lang('send_message'), ["class" => "btn btn-transparent success btn-sm", "title" => lang('send_message')]);
			  }
		  }
		  ?>
      </div>
