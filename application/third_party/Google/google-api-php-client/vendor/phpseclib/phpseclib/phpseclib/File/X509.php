...
class X509
{
	...
	function _mapOutAttributes(&$root, $path, $asn1)
	{
		...
		if(is_array($attributes))
		{
			...
			for($i = 0; $i < $size; $i++)
			{
				...
				else if(is_array($attributes[$i]['value']))
				{
					$values = &$attributes[$i]['value'];
					for($j = 0; $j < count($values); $j++)
					{
	...
	function _validateSignatureCountable($caonly, $count)
	{
		...
		switch(true)
		{
			case isset($this->currentCert['tbsCertificate']):
				...
				else if(!isset($signingCert) || $caonly)
				{
					return $this->_testForIntermediate($caonly, $count) && $this->validateSignature($caonly);
				}
		...
		for($i = 0; $i < count($dn); $i++)
		{
			if($dn[$i][0]['type'] == $propName)
			{
				...
				if(!$withType)
				{
					...
					else if(is_object($v) && $v instanceof Element)
					{
	...
	function getDN($format = self::DN_ARRAY, $dn = NULL)
	{
		...
		foreach($dn['rdnSequence'] as $field)
		{
			...
			else if(is_object($value) && $value instanceof Element)
			{
				$callback = function($x)
				{
	...
	function signCRL($issuer, $crl, $signatureAlgorithm = 'sha1WithRSAEncryption')
	{
		...
		if(!$version)
		{
			...
			else if(!empty($tbsCertList['revokedCertificates']))
			{
