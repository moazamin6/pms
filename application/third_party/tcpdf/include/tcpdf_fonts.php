...
class TCPDF_FONTS
{
	...
	public static function getFontFullPath($file, $fontdir = false)
	{
		...
		else if(@file_exists(self::_getfontpath() . $file))
		{
	...
	public static function UTF8ArrToLatin1Arr($unicode)
	{
		...
		foreach($unicode as $char)
		{
			...
			else if(array_key_exists($char, TCPDF_FONT_DATA::$uni_utf8tolatin))
			{
				// map from UTF-8
	...
	public static function utf8Bidi($ta, $str = '', $forcertl = false, $isunicode = true, &$currentfont)
	{
		...
		for($i = 0; $i < $numchars; ++$i)
		{
			...
			else if($ta[$i] == TCPDF_FONT_DATA::$uni_LRE)
			{
			...
			else if($ta[$i] == TCPDF_FONT_DATA::$uni_RLO)
			{
			...
			else if($ta[$i] == TCPDF_FONT_DATA::$uni_LRO)
			{
			...
			else if($ta[$i] == TCPDF_FONT_DATA::$uni_PDF)
			{
			...
			else if(($ta[$i] != TCPDF_FONT_DATA::$uni_RLE) and
		...
		for($i = 0; $i < $numchars; ++$i)
		{
			if(($levcount > 0) and (($i + 1) < $numchars) and ($chardata[($i + 1)]['level'] == $prevlevel))
			{
				...
				else if(($chardata[$i]['type'] == 'CS') and ($chardata[($i - 1)]['type'] == 'EN') and ($chardata[($i + 1)]['type'] == 'EN'))
				{
					$chardata[$i]['type'] = 'EN';
				}
				else if(($chardata[$i]['type'] == 'CS') and ($chardata[($i - 1)]['type'] == 'AN') and ($chardata[($i + 1)]['type'] == 'AN'))
				{
					$chardata[$i]['type'] = 'AN';
				}
			}
		...
		for($i = 0; $i < $numchars; ++$i)
		{
			...
			else if(($levcount > 0) and ((($i + 1) == $numchars) or (($i + 1) < $numchars) and ($chardata[($i + 1)]['level'] != $prevlevel)))
			{
				//last char
