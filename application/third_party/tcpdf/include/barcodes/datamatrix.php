	...
	public function __construct($code)
	{
		...
		else if($params[11] > $nd)
		{
			...
			if((($params[11] - $nd) > 1) and ($cw[($nd - 1)] != 254))
			{
				...
				else if(($this->last_enc != ENC_ASCII) and ($this->last_enc != ENC_BASE256))
				{
					// switch to ASCII encoding
					$cw[] = 254;
					++$nd;
				...
				{ // ANSI X12
					...
					do
					{
						...
						else
						{
							...
							else if(isset($chr, $this->chset['SH2'][$chr]))
							{
								$temp_cw[] = 1; // shift 2
								$shiftset = $this->chset['SH2'];
							}
							else if(($enc == ENC_C40) and isset($this->chset['S3C'][$chr]))
							{
								$temp_cw[] = 2; // shift 3
								$shiftset = $this->chset['S3C'];
							}
							else if(($enc == ENC_TXT) and isset($this->chset['S3T'][$chr]))
							{
								$temp_cw[] = 2; // shift 3
