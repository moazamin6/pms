...
class TCPDF_PARSER
{
	...
	protected function getXrefData($offset = 0, $xref = [])
	{
		...
		else if(strpos($this->pdfdata, 'xref', $offset) == $offset)
		{
			// Already pointing at the xref table
			$startxref = $offset;
		}
		else if(preg_match('/([0-9]+[\s][0-9]+[\s]obj)/i', $this->pdfdata, $matches, PREG_OFFSET_CAPTURE, $offset))
		{
	...
	protected function decodeXrefStream($startxref, $xref = [])
	{
		...
		foreach($sarr as $k => $v)
		{
			...
			else if(($v[0] == '/') and ($v[1] == 'Index') and (isset($sarr[($k + 1)])))
			{
			...
			else if(($v[0] == '/') and ($v[1] == 'Prev') and (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'numeric')))
			{
				// get previous xref offset
				$prevxref = intval($sarr[($k + 1)][1]);
			}
			else if(($v[0] == '/') and ($v[1] == 'W') and (isset($sarr[($k + 1)])))
			{
			...
			else if(($v[0] == '/') and ($v[1] == 'DecodeParms') and (isset($sarr[($k + 1)][1])))
			{
				$decpar = $sarr[($k + 1)][1];
				foreach($decpar as $kdc => $vdc)
				{
					...
					else if(($vdc[0] == '/') and ($vdc[1] == 'Predictor') and (isset($decpar[($kdc + 1)]) and ($decpar[($kdc + 1)][0] == 'numeric')))
					{
			...
			else if($filltrailer)
			{
				...
				else if(($v[0] == '/') and ($v[1] == 'Root') and (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'objref')))
				{
					$xref['trailer']['root'] = $sarr[($k + 1)][1];
				}
				else if(($v[0] == '/') and ($v[1] == 'Info') and (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'objref')))
				{
					$xref['trailer']['info'] = $sarr[($k + 1)][1];
				}
				else if(($v[0] == '/') and ($v[1] == 'Encrypt') and (isset($sarr[($k + 1)]) and ($sarr[($k + 1)][0] == 'objref')))
				{
					$xref['trailer']['encrypt'] = $sarr[($k + 1)][1];
				}
				else if(($v[0] == '/') and ($v[1] == 'ID') and (isset($sarr[($k + 1)])))
				{
					$xref['trailer']['id'] = [];
		...
		switch($char)
		{
			...
			{ // \x3E GREATER-THAN SIGN
				...
				else
				{
					...
					else if(($endpos = strpos($this->pdfdata, '>', $offset)) !== false)
					{
						$offset = $endpos + 1;
					}
				}
