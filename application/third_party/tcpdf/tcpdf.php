...
class TCPDF
{
	...
	public function SetFontSize($size, $out = true)
	{
		...
		else if(!isset($font_descent))
		{
			$font_descent = $font_height - $font_ascent;
		}
		else if(!isset($font_ascent))
		{
						...
						for($i = 0; $i < $numchars; $i++)
						{
							if(($unicode[$i] >= 0x0e00) && ($unicode[$i] <= 0x0e5b))
							{
								...
								if(in_array($ch0, $tonemark))
								{
									...
									else if(in_array($ch1, $longtail) or (in_array($ch2, $longtail) and in_array($ch1, $lowvowel)))
									{
								...
								else if(($ch0 == 0x0e33) and (in_array($ch1, $longtail) or (in_array($ch2, $longtail) and in_array($ch1, $tonemark))))
								{
	...
	protected function getCellBorder($x, $y, $w, $h, $brd)
	{
		...
		foreach($brd as $border => $style)
		{
			...
			else if(strlen($border) == 2)
			{
				...
				else if((strpos($border, 'T') !== false) and (strpos($border, 'R') !== false))
				{ // TR
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
				...
				else if((strpos($border, 'R') !== false) and (strpos($border, 'B') !== false))
				{ // RB
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
				...
				else if((strpos($border, 'B') !== false) and (strpos($border, 'L') !== false))
				{ // BL
					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
				...
				else if((strpos($border, 'L') !== false) and (strpos($border, 'R') !== false))
				{ // LR
					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				...
				else if((strpos($border, 'T') !== false) and (strpos($border, 'B') !== false))
				{ // TB
					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
	...
	public function Image($file, $x = '', $y = '', $w = 0, $h = 0, $type = '', $link = '', $align = '', $resize = false, $dpi = 300, $palign = '', $ismask = false, $imgmask = false, $border = 0, $fitbox = false, $hidden = false, $fitonpage = false, $alt = false, $altimgs = [])
	{
		...
		{ // image file
			...
			if(($imsize = @getimagesize($file)) === false)
			{
				...
				else if(strpos($file, '__tcpdf_' . $this->file_id . '_img') === false)
				{
		...
		else if(($ismask === false) and ($imgmask === false) and (strpos($file, '__tcpdf_' . $this->file_id . '_imgmask_') === false))
		{
				...
				foreach($this->PageAnnots[$n] as $key => $pl)
				{
					...
					switch(strtolower($pl['opt']['subtype']))
					{
						...
						case 'link':
						{
							...
							else if(isset($this->links[$pl['txt']]))
							{
								// internal link ID
								$l = $this->links[$pl['txt']];
				...
				if(isset($info['trns']) and is_array($info['trns']))
				{
					...
					if($info['cs'] == 'Indexed')
					{
						...
						for($i = 0; $i < $count_info; ++$i)
						{
							...
							else if(empty($trns) and ($info['trns'][$i] == 0))
							{
	...
	protected function _putencryption()
	{
		...
		else
		{
			...
			else if($this->encryptdata['V'] == 4)
			{ // AES-128
				$out .= ' 4';
			}
			else if($this->encryptdata['V'] < 2)
			{ // RC-40
				$out .= ' 2';
			}
			else
	...
	protected function _putbookmarks()
	{
		...
		foreach($this->outlines as $i => $o)
		{
			...
			else if(isset($this->page_obj_id[($o['p'])]))
			{
				// link to a page
	...
	public function writeHTML($html, $ln = true, $fill = false, $reseth = false, $cell = false, $align = '')
	{
		...
		while($key < $maxel)
		{
			...
			if($dom[$key]['tag'] or ($key == 0))
			{
				...
				else if(isset($dom[$key]['fontname']) or isset($dom[$key]['fontstyle']) or isset($dom[$key]['fontsize']) or isset($dom[$key]['line-height']))
				{
			...
			if($dom[$key]['tag'])
			{
				if($dom[$key]['opening'])
				{
					...
					if(($dom[$key]['value'] == 'td') or ($dom[$key]['value'] == 'th'))
					{
						...
						else if($this->current_column != $dom[$trid]['startcolumn'])
						{
							$tmpx = $this->x;
						...
						if(isset($dom[$table_el]['rowspans']))
						{
							...
							while($rsk < $rskmax)
							{
								...
								if($trwsp['startpage'] < $this->page)
								{
									...
									else if((!$this->rtl) and ($this->pagedim[$this->page]['olm'] != $this->pagedim[$trwsp['startpage']]['olm']))
									{
						...
						if($rowspan <= 1)
						{
							if(isset($dom[$trid]['endy']))
							{
								...
								else if(($this->page > $dom[$trid]['endpage']) or ($this->current_column > $dom[$trid]['endcolumn']))
								{
							...
							foreach($dom[$table_el]['rowspans'] as $k => $trwsp)
							{
								if($trwsp['rowspan'] > 0)
								{
									if(isset($dom[$trid]['endpage']))
									{
										...
										else if(($trwsp['endpage'] < $dom[$trid]['endpage']) or ($trwsp['endcolumn'] < $dom[$trid]['endcolumn']))
										{
		...
		switch($tag['value'])
		{
			...
			case 'li':
			{
				...
				if($this->listordered[$this->listnum])
				{
					...
					else if(isset($parent['listtype']) and !TCPDF_STATIC::empty_string($parent['listtype']))
					{
						$this->lispacer = $parent['listtype'];
					}
					else if(isset($this->lisymbol) and !TCPDF_STATIC::empty_string($this->lisymbol))
					{
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney = 0)
	{
		...
		switch($tag['value'])
		{
			case 'tr':
			{
				...
				if(isset($dom[$table_el]['rowspans']))
				{
					foreach($dom[$table_el]['rowspans'] as $k => $trwsp)
					{
						...
						if($dom[$table_el]['rowspans'][$k]['rowspan'] == 0)
						{
							...
							else if(($dom[$table_el]['rowspans'][$k]['endpage'] > $parent['endpage']) or ($dom[$table_el]['rowspans'][$k]['endcolumn'] > $parent['endcolumn']))
							{
				...
				else if(isset($dom[$table_el]['border-spacing']))
				{
				...
				if($this->page > $parent['startpage'])
				{
					...
					else if((!$this->rtl) and ($this->pagedim[$this->page]['olm'] != $this->pagedim[$parent['startpage']]['olm']))
					{
			...
			case 'table':
			{
				...
				foreach($table_el['trids'] as $j => $trkey)
				{
					...
					else if(isset($table_el['border-spacing']))
					{
					...
					if($endpage > $startpage)
					{
						...
						else if((!$this->rtl) and ($this->pagedim[$endpage]['olm'] != $this->pagedim[$startpage]['olm']))
						{
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x = 0, $y = 0, $w = 1, $h = 1, $clip_function = '', $clip_params = [])
	{
		...
		if($svgstyle['stroke'] != 'none')
		{
			...
			else if(preg_match('/rgba\(\d+%?,\s*\d+%?,\s*\d+%?,\s*(\d+(?:\.\d+)?)\)/i', $svgstyle['stroke'], $rgba_matches))
			{
		...
		switch($font_weight)
		{
			...
			case 'normal':
			{
				...
				else if(substr($font_family, -1) == 'B')
				{
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm = [])
	{
		...
		foreach($prev_svgstyle as $key => $val)
		{
			...
			else if(isset($attribs['style']) and !TCPDF_STATIC::empty_string($attribs['style']))
			{
				// CSS style syntax
				$attrval = [];
		...
		switch($name)
		{
			...
			case 'svg':
			{
				...
				if(isset($attribs['viewBox']))
				{
					...
					if(sizeof($tmp) == 4)
					{
						...
						else if((($fit == 'meet') and ($hr > $wr)) or (($fit == 'slice') and ($hr < $wr)))
						{
							if($aspectY == 'YMax')
							{
								$ay = (($vh * ($hr / $wr)) - $vh);
			...
			case 'radialGradient':
			{
				...
				else if(isset($attribs['r']) and is_numeric($attribs['r']) and ($attribs['r']) <= 1)
				{
