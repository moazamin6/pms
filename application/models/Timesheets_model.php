...
class Timesheets_model extends Crud_model
{
	...
	function get_summary_details($options = [])
	{
		$timesheet_table = $this->db->dbprefix('project_time');
		$tasks_table = $this->db->dbprefix('tasks');
		$projects_table = $this->db->dbprefix('projects');
	...
	{
		return parent::get_all_where(["project_id" => $project_id, "user_id" => $user_id, "status" => "open", "deleted" => 0]);
	}
	
	function get_task_timer_info($task_id, $user_id)
	{
		return parent::get_all_where(["task_id" => $task_id, "user_id" => $user_id, "status" => "open", "deleted" => 0]);
	}
	...
	function process_timer($data)
	{
		...
		$where = ["project_id" => $project_id, "user_id" => $user_id, "status" => "open", "deleted" => 0];
		
		if($task_id)
		{
			//check if there already has any task
			$task_where = array_merge($where, ["task_id" => $task_id]);
	...
	function get_timesheet_statistics($options = [])
	{
		$timesheet_table = $this->db->dbprefix('project_time');
	...
	function active_members_on_projects()
	{
		$users_table = $this->db->dbprefix('users');
		$projects_table = $this->db->dbprefix('projects');
		$project_time_table = $this->db->dbprefix('project_time');
	...
	function count_total_time($options = [])
	{
