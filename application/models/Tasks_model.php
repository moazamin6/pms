...
class Tasks_model extends Crud_model
{
	...
	private function make_quick_filter_query($filter, $tasks_table)
	{
		...
		else
		{
			...
			{
				if($filter === "mentioned_me" && isset($this->login_user->id))
				{
				...
				{
					if($filter === "recently_mentioned_me" && isset($this->login_user->id))
					{
						$mention_string = ":" . $this->login_user->id . "]";
	...
	function get_kanban_details($options = [])
	{
		$tasks_table = $this->db->dbprefix('tasks');
		$users_table = $this->db->dbprefix('users');
		$projects = $this->db->dbprefix('projects');
	...
	function get_task_statistics($options = [])
	{
		$tasks_table = $this->db->dbprefix('tasks');
		$task_status_table = $this->db->dbprefix('task_status');
	...
	function save_reminder_date(&$data = [], $id = 0)
	{
		if($id)
		{
			$where = ["id" => $id];
	...
	function update_custom_data(&$data = [], $id = 0)
	{
		if($id)
		{
			$where = ["id" => $id];
	...
	function get_search_suggestion($search = "", $options = [])
	{
		$tasks_table = $this->db->dbprefix('tasks');
