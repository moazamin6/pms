...
class Events_model extends Crud_model
{
	...
	function count_events_today($options = [])
	{
		$events_table = $this->db->dbprefix('events');
	...
	function get_upcomming_events($options = [])
	{
		...
		if($has_recurring)
		{
			usort($final_result, [$this, "sort_by_start_date"]); //sort by start date
		...
		if($event_info)
		{
			...
			else
			{
				...
				foreach($share_with_array as $share)
				{
					...
					{
						if(get_array_value($share_data, '0') === "team")
						{
