...
class Project_members_model extends Crud_model
{
	...
	function save_member($data = [], $id = 0)
	{
		...
		$exists = $this->get_one_where($where = ["user_id" => $user_id, "project_id" => $project_id]);
		// print_r($exists);
	...
	function get_project_members_dropdown_list($project_id = 0, $user_ids = [], $add_client_contacts = false, $show_active_users_only = false)
	{
		$project_members_table = $this->db->dbprefix('project_members');
		$users_table = $this->db->dbprefix('users');
	...
	{
		$info = $this->get_one_where(["project_id" => $project_id, "user_id" => $user_id, "deleted" => 0]);
		if($info->id)
		{
			return true;
		}
