...
class Users_model extends Crud_model
{
	...
	function authenticate($email, $password)
	{
		...
		$result = $this->db->get_where($this->table, ['email' => $email, 'status' => 'active', 'deleted' => 0, 'disable_login' => 0]);
	...
	function get_job_info($user_id)
	{
		...
		return parent::get_one_where(["user_id" => $user_id]);
	}
	...
	function save_job_info($data)
	{
		...
		$where = ["user_id" => get_array_value($data, "user_id")];
	...
	function get_active_members_and_clients($options = [])
	{
		$users_table = $this->db->dbprefix('users');
		$clients_table = $this->db->dbprefix('clients');
