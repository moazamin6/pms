...
if(!function_exists('activity_logs_widget'))
{
	...
	function activity_logs_widget($params = [], $return_as_data = false)
	{
		$ci = get_instance();
...
if(!function_exists('timeline_widget'))
{
	...
	function timeline_widget($params = [], $return_as_data = false)
	{
		$ci = get_instance();
...
if(!function_exists('my_task_stataus_widget'))
{
	...
	{
		$ci = get_instance();
		$view_data["task_statuses"] = $ci->Tasks_model->get_task_statistics(["user_id" => $ci->login_user->id]);
...
if(!function_exists('events_widget'))
{
	...
	function events_widget($return_as_data = false)
	{
		...
		$options = ["user_id" => $ci->login_user->id, "limit" => 10, "team_ids" => $ci->login_user->team_ids];
...
if(!function_exists('invoice_statistics_widget'))
{
	...
	function invoice_statistics_widget($return_as_data = false, $options = [])
	{
		$ci = get_instance();
...
if(!function_exists('all_tasks_kanban_widget'))
{
	...
	function all_tasks_kanban_widget($return_as_data = false)
	{
		...
		$assigned_to_list = $ci->Users_model->get_dropdown_list(["first_name", "last_name"], "id", ["deleted" => 0, "user_type" => "staff"]);
...
if(!function_exists('sticky_note_widget'))
{
	...
	{
		$ci = get_instance();
		return $ci->load->view("dashboards/sticky_note_widget", ["custom_class" => $custom_class], $return_as_data);
	}
...
if(!function_exists('ticket_status_widget_small'))
{
	...
	function ticket_status_widget_small($data = [], $return_as_data = false)
	{
		$ci = get_instance();
...
if(!function_exists('clocked_in_team_members_widget'))
{
	...
	function clocked_in_team_members_widget($data = [], $return_as_data = false)
	{
		$ci = get_instance();
...
if(!function_exists('clocked_out_team_members_widget'))
{
	...
	function clocked_out_team_members_widget($data = [], $return_as_data = false)
	{
		$ci = get_instance();
...
if(!function_exists('active_members_and_clients_widget'))
{
	...
	function active_members_and_clients_widget($user_type = "", $return_as_data = false, $show_own_clients_only_user_id = "")
	{
		...
		$options = ["user_type" => $user_type, "exclude_user_id" => $ci->login_user->id, "show_own_clients_only_user_id" => $show_own_clients_only_user_id];
...
if(!function_exists('pending_leave_approval_widget'))
{
	...
	function pending_leave_approval_widget($return_as_data = false, $data = [])
	{
		$ci = get_instance();
