...
if(!function_exists('get_notification_config'))
{
	...
	function get_notification_config($event = "", $key = "", $info_options = [])
	{
		...
		$task_link = function($options)
		{
			...
			{
				return ["url" => $url, "ajax_modal_url" => $ajax_url, "large_modal" => "1", "id" => $id];
			}
			else
			{
				//return all tasks link for reminder notifications
				return ["url" => get_uri("projects/all_tasks")];
			}
		};
		...
		$project_link = function($options)
		{
			...
			if(isset($options->project_id))
			{
				...
				{
					if($options->event == "project_comment_added" || $options->event == "project_comment_replied")
					{
						$url .= "/comment";
					}
				}
		...
		$project_file_link = function($options)
		{
			...
			return ["url" => $url, "app_modal_url" => $app_modal_url, "id" => $id];
		};
		...
		$leave_link = function($options)
		{
			...
			return ["url" => $url, "ajax_modal_url" => $ajax_url, "id" => $id];
		};
		...
		$events = [
			"project_created"                              => [
				"notify_to" => ["client_primary_contact", "client_all_contacts", "team_members", "team"],
				"info"      => $project_link,
			],
			"project_completed"                            => [
				"notify_to" => ["client_primary_contact", "client_all_contacts", "client_assigned_contacts", "team_members", "team"],
				"info"      => $project_link,
			],
			"project_deleted"                              => [
				"notify_to" => ["project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "team_members", "team"],
			],
			"project_task_created"                         => [
				"notify_to" => ["project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "task_assignee", "task_collaborators", "team_members", "team"],
				"info"      => $task_link,
			],
			...
			"project_task_assigned"                        => [
				"notify_to" => ["project_members", "task_assignee", "task_collaborators", "team_members", "team"],
				"info"      => $task_link,
			],
			...
			"project_task_commented"                       => [
				"notify_to" => ["mentioned_members", "project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "task_assignee", "task_collaborators", "team_members", "team"],
				"info"      => $task_link,
			],
			...
			"project_member_added"                         => [
				"notify_to" => ["project_members", "team_members", "team"],
				"info"      => $project_link,
			],
			...
			"project_file_commented"                       => [
				"notify_to" => ["mentioned_members", "project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "team_members", "team"],
				"info"      => $project_file_link,
			],
			"project_comment_added"                        => [
				"notify_to" => ["mentioned_members", "project_members", "team_members", "team"],
				"info"      => $project_link,
			],
			"project_comment_replied"                      => [
				"notify_to" => ["mentioned_members", "project_members", "comment_creator", "team_members", "team"],
				"info"      => $project_link,
			],
			"project_customer_feedback_added"              => [
				"notify_to" => ["mentioned_members", "project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "team_members", "team"],
				"info"      => $project_link,
			],
			"project_customer_feedback_replied"            => [
				"notify_to" => ["mentioned_members", "project_members", "client_primary_contact", "client_all_contacts", "client_assigned_contacts", "comment_creator", "team_members", "team"],
				"info"      => $project_link,
			],
			"client_signup"                                => [
				"notify_to" => ["team_members", "team"],
				"info"      => $client_link,
			],
			"client_contact_requested_account_removal"     => [
				"notify_to" => ["team_members", "team"],
				"info"      => $client_link,
			],
			"invoice_online_payment_received"              => [
				"notify_to" => ["team_members", "team"],
				"info"      => $invoice_link,
			],
			"invoice_payment_confirmation"                 => [
				"notify_to" => ["client_primary_contact", "client_all_contacts"],
				"info"      => $invoice_link,
			],
			"recurring_invoice_created_vai_cron_job"       => [
				"notify_to" => ["client_primary_contact", "client_all_contacts", "team_members", "team"],
				"info"      => $invoice_link,
			],
			...
			"recurring_invoice_creation_reminder"          => [
				"notify_to" => ["client_primary_contact", "client_all_contacts", "team_members", "team"],
				"info"      => $invoice_link,
			],
			...
			"leave_approved"                               => [
				"notify_to" => ["leave_applicant", "team_members", "team"],
				"info"      => $leave_link,
			],
			"leave_assigned"                               => [
				"notify_to" => ["leave_applicant", "team_members", "team"],
			...
			"ticket_created"                               => [
				"notify_to" => ["client_primary_contact", "client_all_contacts", "ticket_creator", "ticket_assignee", "team_members", "team"],
				"info"      => $ticket_link,
			],
			"ticket_commented"                             => [
				"notify_to" => ["client_primary_contact", "client_all_contacts", "ticket_creator", "ticket_assignee", "team_members", "team"],
			...
			"bitbucket_push_received"                      => [
				"notify_to" => ["project_members", "client_primary_contact", "client_all_contacts", "task_assignee", "task_collaborators", "team_members", "team"],
				"info"      => $task_link,
			],
			...
			"project_task_deadline_pre_reminder"           => [
				"notify_to" => ["task_assignee", "project_members", "team_members", "team"],
				"info"      => $task_link,
			],
			...
			"recurring_task_created_via_cron_job"          => [
				"notify_to" => ["project_members", "client_primary_contact", "client_all_contacts", "task_assignee", "task_collaborators", "team_members", "team"],
				"info"      => $task_link,
			],
			"lead_created"                                 => [
				"notify_to" => ["owner", "team_members", "team"],
				"info"      => $lead_link,
			],
			...
			"order_status_updated"                         => [
				"notify_to" => ["client_primary_contact", "client_all_contacts", "order_creator_contact", "team_members", "team"],
				"info"      => $order_link,
			],
			"timeline_post_commented"                      => [
				"notify_to" => ["post_creator", "team_members", "team"],
				"info"      => $timeline_link,
			],
			"created_a_new_post"                           => [
				"notify_to" => ["team_members", "team"],
			...
			"invited_client_contact_signed_up"             => [
				"notify_to" => ["team_members", "team"],
				"info"      => $client_link,
			],
			"ticket_assigned"                              => [
				"notify_to" => ["ticket_assignee", "team_members", "team"],
				"info"      => $ticket_link,
			],
		];
...
if(!function_exists('send_notification_emails'))
{
	...
	function send_notification_emails($notification_id, $email_notify_to, $extra_data = [])
	{
		...
		if($notification->category == "ticket" && $notification->event !== "ticket_assigned")
		{
			...
			//add attachment
			$comments_options = ["id" => $notification->ticket_comment_id];
		...
		else
		{
			...
			{
				if($notification->event == "new_message_sent" || $notification->event == "message_reply_sent")
				{
					$email_template = $ci->Email_templates_model->get_final_template("message_received");
					
					$message_info = $ci->Messages_model->get_details(["id" => $notification->actual_message_id])->row;
					
					//reply? find the subject from the parent meessage
					if($notification->event == "message_reply_sent")
					{
						$main_message_info = $ci->Messages_model->get_details(["id" => $message_info->message_id])->row;
				...
				{
					if($notification->event == "recurring_invoice_created_vai_cron_job" || $notification->event == "invoice_due_reminder_before_due_date" || $notification->event == "invoice_overdue_reminder" || $notification->event == "recurring_invoice_creation_reminder")
					{
						...
						{
							if($notification->event == "invoice_due_reminder_before_due_date")
							{
								$email_template = $ci->Email_templates_model->get_final_template("invoice_due_reminder_before_due_date");
							}
							else
							{
								if($notification->event == "invoice_overdue_reminder")
								{
						...
						$email_options["attachments"] = [["file_path" => $attachement_url]];
						...
						{
							$invoice_status_data = ["status" => "not_paid"];
					...
					{
						if($notification->category == "estimate")
						{
							...
							else
							{
								...
								{
									if($notification->event == "estimate_accepted")
									{
		...
		if($notification_multiple_tasks_user_wise && ($notification->event == "project_task_deadline_pre_reminder" || $notification->event == "project_task_reminder_on_the_day_of_deadline" || $notification->event == "project_task_deadline_overdue_reminder"))
		{
			...
			foreach($notification_multiple_tasks_user_wise as $user_id => $tasks)
			{
				...
				$table = $ci->load->view("projects/tasks/notification_multiple_tasks_table", ["tasks" => $tasks], true);
...
if(!function_exists('get_notification_url_attributes'))
{
	...
	function get_notification_url_attributes($notification)
	{
		...
		return ["url_attributes" => $url_attributes, "url" => $url];
	}
...
if(!function_exists('get_notification_multiple_tasks_data'))
{
	...
	function get_notification_multiple_tasks_data($tasks, $event)
	{
		...
		foreach($tasks as $task)
		{
			...
			{
				$options = ["project_id" => $task->project_id];
	...
	function send_slack_notification($event, $user_id = 0, $notification_id = 0, $webhook_url = "")
	{
		if($webhook_url)
		{
			...
			if($notification_id)
			{
				...
				$notification_description = $ci->load->view("notifications/notification_description_for_slack", ["notification" => $notification_info], true);
			}
