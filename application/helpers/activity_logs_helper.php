...
if(!function_exists('get_change_logs'))
{
	...
	function get_change_logs($log_type, $field, $value)
	{
		...
		else
		{
			...
			{
				if($log_type === "project_comment")
				{
					$model_schema = $ci->Project_comments_model->schema();
				}
				else
				{
					if($log_type === "project_file")
					{
						$model_schema = $ci->Project_files_model->schema();
					}
					else
					{
						if($log_type === "file_comment")
						{
							$model_schema = $ci->Project_comments_model->schema();
						}
						else
						{
							if($log_type === "task_comment")
							{
								$model_schema = $ci->Project_comments_model->schema();
							}
						}
		...
		if($schema_info)
		{
			...
			{
				if(get_array_value($schema_info, "type") === "text")
				{
				...
				{
					if(get_array_value($schema_info, "type") === "foreign_key")
					{
						...
						if($from_value && $linked_model)
						{
							...
							{
								if(get_array_value($schema_info, "link_type") === "label_group_list")
								{
					...
					{
						if(get_array_value($schema_info, "type") === "language_key")
						{
							$changes = "<del>" . lang($from_value) . "</del> <ins>" . lang($to_value) . "</ins>";
						}
						else
						{
							if(get_array_value($schema_info, "type") === "date")
							{
							...
							{
								if(get_array_value($schema_info, "type") === "time")
								{
									$changes = "<del>" . $from_value . "</del> <ins>" . $to_value . "</ins>";
								}
								else
								{
									if(get_array_value($schema_info, "type") === "date_time")
									{
										$changes = "<del>" . $from_value . "</del> <ins>" . $to_value . "</ins>";
									}
									else
...
if(!function_exists('get_logs_of_bitbucket_or_github_commit'))
{
	...
	function get_logs_of_bitbucket_or_github_commit($changes = [], $is_notification = false, $service_type = "bitbucket")
	{
		...
		$author_link = anchor(get_array_value($commit_info, "author_link"), get_array_value($commit_info, "author_name"), ["target" => "_blank"]);
		...
		if($commits_count)
		{
			...
			if($commits_count > 1)
			{
				$commits_text = strtolower(lang("new_commits"));
			}
			else
			{
				$commits_text = strtolower(lang("new_commit"));
			}
		}
		...
		if($is_notification)
		{
			array_push($changes_array, "[" . $branch_name_with_repository . "] " . $commits_amount . " " . $commits_text . " " . strtolower(lang("pushed_by")) . " " . $author_name);
		}
		else
		{
			array_push($changes_array, "[<strong>" . $branch_name_with_repository . "</strong>] " . $commits_amount . " " . $commits_text . " " . strtolower(lang("pushed_by")) . " <strong>" . ($is_notification ? $author_name : $author_link) . "</strong>");
		}

		foreach($commits as $commit)
		{
			$commit_message = get_array_value($commit, "commit_message");
			array_push($changes_array, "<li>" . ($is_notification ? $commit_message : anchor(get_array_value($commit, "commit_url"), $commit_message, ["target" => "_blank"])) . "</li>");
		}
	...
	function get_change_logs_array($changes, $log_type, $action = "", $is_notification = false)
	{
		...
		if($changes !== "")
		{
			...
			if(is_array($changes))
			{
				...
				{
					if($action === "github_notification_received" || $action === "github_push_received")
					{
						$changes_array = get_logs_of_bitbucket_or_github_commit($changes, $is_notification, "github");
					}
					else
					{
						if($action === "all" || $action === "updated")
						{
