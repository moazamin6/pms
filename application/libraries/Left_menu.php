...
class Left_menu
{
	...
	private function _prepare_sidebar_menu_items($type = "")
	{
		...
		//add todo
		$final_items_array["todo"] = ["name" => "todo", "url" => "todo", "class" => "fa-check-square-o"];
	...
	private function _get_left_menu_from_setting_for_rander($is_preview = false, $type = "default")
	{
		...
		return $custom_left_menu ? json_decode(json_encode(@unserialize($custom_left_menu)), true) : [];
	}
	...
	private function _get_left_menu_from_setting($type)
	{
		...
		return $default_left_menu ? json_decode(json_encode(@unserialize($default_left_menu)), true) : [];
	}
	...
	public function _get_item_data($item, $is_default_item = false)
	{
		...
		if($name)
		{
			...
			else
			{
				...
				$name_lang = $name;
				$edit_button = modal_anchor(get_uri("left_menus/add_menu_item_modal_form"), "<i class='fa fa-pencil'></i> ", ["title" => lang('edit'), "class" => "custom-menu-edit-button", "data-post-title" => $name, "data-post-url" => $url, "data-post-is_sub_menu" => $is_sub_menu, "data-post-icon" => $icon, "data-post-open_in_new_tab" => $open_in_new_tab]);
			}
	...
	private function _get_item_array_value($data_array, $left_menu_items)
	{
		...
		if($url)
		{ //custom menu item
			$item_value_array = ["name" => $name, "url" => $url, "is_custom_menu_item" => true, "class" => "fa-$icon", "open_in_new_tab" => $open_in_new_tab];
		}
		else
		{
			if(array_key_exists($name, $left_menu_items))
			{ //default menu items
	...
	{
		$dashboard_menu = ["name" => "dashboard", "url" => "dashboard", "class" => "fa-desktop dashboard-menu"];

		$selected_dashboard_id = get_setting("user_" . $this->ci->login_user->id . "_dashboard");
		if($selected_dashboard_id)
		{
			$dashboard_menu = ["name" => "dashboard", "url" => "dashboard/view/" . $selected_dashboard_id, "class" => "fa-desktop dashboard-menu"];
		}
		...
		if($this->ci->login_user->user_type == "staff" && $type !== "client_default")
		{
			...
			{
				$sidebar_menu["timeline"] = ["name" => "timeline", "url" => "timeline", "class" => " fa-comments font-18"];
			}

			if(get_setting("module_event") == "1")
			{
				$sidebar_menu["events"] = ["name" => "events", "url" => "events", "class" => "fa-calendar"];
			}

			if(get_setting("module_note") == "1")
			{
				$sidebar_menu["notes"] = ["name" => "notes", "url" => "notes", "class" => "fa-book font-16"];
			}

			if(get_setting("module_message") == "1" && $access_messages)
			{
				$sidebar_menu["messages"] = ["name" => "messages", "url" => "messages", "class" => "fa-envelope", "devider" => true, "badge" => count_unread_message(), "badge_class" => "badge-secondary"];
			}


			if($this->ci->login_user->is_admin || $access_client)
			{
				$sidebar_menu["clients"] = ["name" => "clients", "url" => "clients", "class" => "fa-briefcase"];
			}

			if(get_setting("module_lead") == "1" && ($this->ci->login_user->is_admin || $access_lead))
			{
				$sidebar_menu["leads"] = ["name" => "leads", "url" => "leads", "class" => "fa-cubes"];
			}

			$project_submenu = [
				["name" => "all_projects", "url" => "projects/all_projects"],
				["name" => "tasks", "url" => "projects/all_tasks", "class" => "fa-tasks"],
			];

			if(get_setting("module_gantt"))
			{
				$project_submenu[] = ["name" => "gantt", "url" => "projects/all_gantt", "class" => "fa-braille"];
			}


			if(get_setting("module_project_timesheet"))
			{
				$project_submenu[] = ["name" => "timesheets", "url" => "projects/all_timesheets", "class" => "fa-clock-o"];
			}
			...
			if(get_setting("module_estimate") && get_setting("module_estimate_request") && ($this->ci->login_user->is_admin || $access_estimate))
			{
				...
				$sidebar_menu["estimates"] = [
					"name"    => "estimates", "url" => "estimates", "class" => "fa-file",
					"submenu" => [
						["name" => "estimate_list", "url" => "estimates"],
						["name" => "estimate_requests", "url" => "estimate_requests", "class" => "fa-file-text-o"],
						["name" => "estimate_forms", "url" => "estimate_requests/estimate_forms", "class" => "fa-file-o"],
					],
				];
			}
			else
			{
				...
				{
					$sidebar_menu["estimates"] = ["name" => "estimates", "url" => "estimates", "class" => "fa-file"];
				}
			}

			if(get_setting("module_invoice") == "1" && ($this->ci->login_user->is_admin || $access_invoice))
			{
				$sidebar_menu["invoices"] = ["name" => "invoices", "url" => "invoices", "class" => "fa-file-text"];
			}

			if($access_items && (get_setting("module_invoice") == "1" || get_setting("module_estimate") == "1"))
			{
				$sidebar_menu["items"] = ["name" => "items", "url" => "items", "class" => "fa-list-ul"];
			}

			if(get_setting("module_order") == "1" && ($this->ci->login_user->is_admin || $access_order))
			{
				$order_submenu = [];
				$order_submenu[] = ["name" => "orders_list", "url" => "orders"];
				$order_submenu[] = ["name" => "sales", "url" => "items/grid_view", "class" => "fa-list-ul"];

				$sidebar_menu["orders"] = ["name" => "orders", "url" => "orders", "class" => "fa-ship", "submenu" => $order_submenu];
			}
			...
			if((get_setting("module_invoice") == "1" || get_setting("module_expense") == "1") && ($this->ci->login_user->is_admin || $access_expense || $access_invoice))
			{
				...
				if(get_setting("module_invoice") == "1" && ($this->ci->login_user->is_admin || $access_invoice))
				{
					$finance_submenu[] = ["name" => "invoice_payments", "url" => "invoice_payments", "class" => "fa-money"];
				...
				{
					$finance_submenu[] = ["name" => "expenses", "url" => "expenses", "class" => "fa-money"];
				...
				{
					$finance_submenu[] = ["name" => "income_vs_expenses", "url" => "expenses/income_vs_expenses", "class" => "fa-line-chart"];
				}

				$sidebar_menu["finance"] = ["name" => "finance", "url" => $finance_url, "class" => "fa-money", "submenu" => $finance_submenu];
			}
			...
			if(get_setting("module_ticket") == "1" && ($this->ci->login_user->is_admin || $access_ticket))
			{
				...
				else
				{
					...
					{
						if($access_ticket === "assigned_only")
						{
				...
				$sidebar_menu["tickets"] = ["name" => "tickets", "url" => "tickets", "class" => "fa-life-ring", "devider" => true, "badge" => $ticket_badge, "badge_class" => "badge-secondary"];
			}


			if(get_array_value($this->ci->login_user->permissions, "hide_team_members_list") != "1")
			{
				$sidebar_menu["team_members"] = ["name" => "team_members", "url" => "team_members/view", "class" => "fa-user font-16"];
			}


			if(get_setting("module_attendance") == "1" && ($this->ci->login_user->is_admin || $access_timecard))
			{
				$sidebar_menu["attendance"] = ["name" => "attendance", "url" => "attendance", "class" => "fa-clock-o font-16"];
			}
			else
			{
				if(get_setting("module_attendance") == "1")
				{
					$sidebar_menu["attendance"] = ["name" => "attendance", "url" => "attendance/attendance_info", "class" => "fa-clock-o font-16"];
				}
			}

			if(get_setting("module_leave") == "1" && ($this->ci->login_user->is_admin || $access_leave))
			{
				$sidebar_menu["leaves"] = ["name" => "leaves", "url" => "leaves", "class" => "fa-sign-out font-16", "devider" => true];
			}
			else
			{
				if(get_setting("module_leave") == "1")
				{
					$sidebar_menu["leaves"] = ["name" => "leaves", "url" => "leaves/leave_info", "class" => "fa-sign-out font-16", "devider" => true];
				}
			}

			if(get_setting("module_announcement") == "1")
			{
				$sidebar_menu["announcements"] = ["name" => "announcements", "url" => "announcements", "class" => "fa-bullhorn"];
			}
			...
			if($module_help || $module_knowledge_base)
			{
				...
				$help_knowledge_base_menues = [];
				$main_url = "help";

				if($module_help)
				{
					$help_knowledge_base_menues[] = ["name" => "help", "url" => $main_url, "class" => "fa-question-circle"];
				}

				//push the help manage menu if user has access
				if($manage_help_and_knowledge_base && $module_help)
				{
					$help_knowledge_base_menues[] = ["name" => "articles", "url" => "help/help_articles", "class" => "fa-question-circle"];
					$help_knowledge_base_menues[] = ["name" => "categories", "url" => "help/help_categories", "class" => "fa-question-circle"];
				}

				if($module_knowledge_base)
				{
					$help_knowledge_base_menues[] = ["name" => "knowledge_base", "url" => "knowledge_base", "class" => "fa-question-circle-o"];
				}

				//push the knowledge_base manage menu if user has access
				if($manage_help_and_knowledge_base && $module_knowledge_base)
				{
					$help_knowledge_base_menues[] = ["name" => "articles", "category" => "help", "url" => "help/knowledge_base_articles", "class" => "fa-question-circle-o"];
					$help_knowledge_base_menues[] = ["name" => "categories", "category" => "help", "url" => "help/knowledge_base_categories", "class" => "fa-question-circle-o"];
				}
			...
			{
				$sidebar_menu["settings"] = ["name" => "settings", "url" => "settings/general", "class" => "fa-wrench"];
			}
		}
		else
		{
			...
			{
				$sidebar_menu[] = ["name" => "events", "url" => "events", "class" => "fa-calendar"];
			}

			//check message access settings for clients
			if(get_setting("module_message") && get_setting("client_message_users"))
			{
				$sidebar_menu[] = ["name" => "messages", "url" => "messages", "class" => "fa-envelope", "badge" => count_unread_message()];
			}

			if(!in_array("projects", $hidden_menu))
			{
				$sidebar_menu[] = ["name" => "projects", "url" => "projects/all_projects", "class" => "fa fa-th-large"];
			}


			if(get_setting("module_estimate") && !in_array("estimates", $hidden_menu))
			{
				$sidebar_menu[] = ["name" => "estimates", "url" => "estimates", "class" => "fa-file"];
			}
			...
			if(get_setting("module_invoice") == "1")
			{
				...
				{
					$sidebar_menu[] = ["name" => "invoices", "url" => "invoices", "class" => "fa-file-text"];
				}
				if(!in_array("payments", $hidden_menu))
				{
					$sidebar_menu[] = ["name" => "invoice_payments", "url" => "invoice_payments", "class" => "fa-money"];
				}
			}

			if(!in_array("store", $hidden_menu) && get_setting("client_can_access_store"))
			{
				$sidebar_menu[] = ["name" => "store", "url" => "items/grid_view", "class" => "fa-shopping-basket"];
				$sidebar_menu[] = ["name" => "orders", "url" => "orders", "class" => "fa-ship"];
			}

			if(get_setting("module_ticket") == "1" && !in_array("tickets", $hidden_menu))
			{
				$sidebar_menu[] = ["name" => "tickets", "url" => "tickets", "class" => "fa-life-ring"];
			}

			if(get_setting("module_announcement") == "1" && !in_array("announcements", $hidden_menu))
			{
				$sidebar_menu[] = ["name" => "announcements", "url" => "announcements", "class" => "fa-bullhorn"];
			}

			$sidebar_menu[] = ["name" => "users", "url" => "clients/users", "class" => "fa-user"];

			if(get_setting("client_can_view_files"))
			{
				$sidebar_menu[] = ["name" => "files", "url" => "clients/files/" . $this->ci->login_user->id . "/page_view", "class" => "fa-file-image-o"];
			}

			$sidebar_menu[] = ["name" => "my_profile", "url" => "clients/contact_profile/" . $this->ci->login_user->id, "class" => "fa-cog"];

			if(get_setting("module_knowledge_base") == "1" && !in_array("knowledge_base", $hidden_menu))
			{
				$sidebar_menu[] = ["name" => "knowledge_base", "url" => "knowledge_base", "class" => "fa-question-circle"];
			}
		}
