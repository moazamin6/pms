...
class Cron_job
{
	...
	private function send_invoice_due_pre_reminder()
	{
		...
		if($reminder_date)
		{
			...
			$invoices = $this->ci->Invoices_model->get_details([
			...
			{
				log_notification("invoice_due_reminder_before_due_date", ["invoice_id" => $invoice->id], "0");
			}
		}
	}
	...
	private function send_invoice_due_after_reminder()
	{
		...
		if($reminder_date)
		{
			...
			foreach($invoices as $invoice)
			{
				log_notification("invoice_overdue_reminder", ["invoice_id" => $invoice->id], "0");
			}
		}
	}
	...
	private function send_recurring_invoice_creation_reminder()
	{
		...
		if($reminder_date)
		{
			...
			foreach($invoices as $invoice)
			{
				log_notification("recurring_invoice_creation_reminder", ["invoice_id" => $invoice->id], "0");
			}
		}
	}
	...
	private function _create_new_invoice($invoice)
	{
		...
		$items = $this->ci->Invoice_items_model->get_details(["invoice_id" => $invoice->id])->result();
		foreach($items as $item)
		{
			//create invoice items for new invoice
			$new_invoice_item_data = [
				"title"       => $item->title,
		...
		log_notification("recurring_invoice_created_vai_cron_job", ["invoice_id" => $new_invoice_id], "0");
	}
	...
	private function _create_new_task($task)
	{
		...
		$checklist_item_options = ["task_id" => $task->id];
		...
		//send notification
		$notification_option = ["project_id" => $task->project_id, "task_id" => $new_task_id];
	...
	{
		$notification_option = ["notification_multiple_tasks" => true];
	...
	private function close_inactive_tickets()
	{
		...
		if($auto_close_ticket_after_days)
		{
			...
			$tickets = $this->ci->Tickets_model->get_details([
			...
			foreach($tickets as $ticket)
			{
				...
				//send notification
				log_notification("ticket_closed", ["ticket_id" => $ticket->id], "0");
			}
		}
