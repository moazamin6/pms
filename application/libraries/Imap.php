...
class Imap
{
	...
	private function _create_ticket_from_imap($message_info = "")
	{
		if($message_info)
		{
			...
			$client_info = $this->ci->Users_model->get_one_where(["email" => $email, "user_type" => "client", "deleted" => 0]);
			...
			if($ticket_id)
			{
				$existing_ticket_info = $this->ci->Tickets_model->get_one_where(["id" => $ticket_id, "deleted" => 0]);
			...
			if($ticket_id)
			{
				...
				{
					log_notification("ticket_commented", ["ticket_id" => $ticket_id, "ticket_comment_id" => $ticket_comment_id, "exclude_ticket_creator" => true], $client_info->id ? $client_info->id : "0");
				}
			}
			else
			{
				...
				if($ticket_id)
				{
					...
					if($ticket_id && $ticket_comment_id)
					{
						log_notification("ticket_created", ["ticket_id" => $ticket_id, "ticket_comment_id" => $ticket_comment_id, "exclude_ticket_creator" => true], $client_info->id ? $client_info->id : "0");
					}
				}
			}
		}
			...
			if(!$description)
			{
				...
				if(!$description)
				{
					...
					else
					{
						...
						{
							if($encoding_type == "quoted-printable")
							{
