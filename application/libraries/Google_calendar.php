...
class Google_calendar
{
	...
	private function _check_calendar_ids($client, $user_id = "")
	{
		...
		if(!$calendar_ids)
		{ //check if user have any additional calendar
			return true;
		}
		...
		foreach($calendar_ids_array as $calendar_id)
		{
			try
			{
				$service->events->listEvents("primary", ["calendarId" => $calendar_id]);
	...
	private function _get_start_end_date_time($event_info, $type = "", $datetime_object_type = "")
	{
		...
		$time_array = ["timeZone" => get_setting("timezone")];

		if($datetime_object_type == "date")
		{ //all day event
			$time_array["date"] = $event_info->$date_object;
		}
		else
		{ //customized time event
	...
	private function _get_recurrence_data_for_google($event_info = "")
	{
		if($event_info && $event_info->recurring)
		{
			...
			{
				if($event_info->repeat_type == "months")
				{
					$repeat_type = "MONTHLY";
				}
				else
				{
					if($event_info->repeat_type == "years")
					{
						$repeat_type = "YEARLY";
					}
				}
			...
			return ["RRULE:FREQ=" . $repeat_type . ";UNTIL=" . gmdate("Ymd\THis\Z", strtotime($event_info->last_start_date)) . ";INTERVAL=" . $event_info->repeat_every];
		}
		else
		{
			return [];
				...
				if($user_id && get_setting('user_' . $user_id . '_oauth_access_token') && get_setting('user_' . $user_id . '_google_calendar_authorized'))
				{
					...
					if($calendar_ids)
					{
						...
						if(count($calendar_ids_array))
						{
							foreach($calendar_ids_array as $calendar_id)
							{
								if($calendar_id != $user_info->email)
								{ //user's private calender id and his email is same
	...
	private function _prepare_calendar_events($events = [], $user_id = 0)
	{
		$recurring_event_ids_array = [];
		$user_info = $this->ci->Users_model->get_access_info($user_id);

		//create/get google calendar label
		$label_data = ["title" => lang("google_calendar_event"), "color" => "#2d9cdb", "context" => "event", "user_id" => $user_id];
		$existing_label = $this->ci->Labels_model->get_one_where(array_merge($label_data, ["deleted" => 0]));
			...
			if($user_info->user_type == "staff" && !get_array_value($permissions, "disable_event_sharing") && $event->attendees && count($event->attendees))
			{
				...
				foreach($event->attendees as $attendee)
				{
					if($attendee->email != $user_info->email)
					{
						...
						$share_with_user_info = $this->ci->Users_model->get_one_where(["email" => $attendee->email, "deleted" => 0]);
	...
	private function _get_repeats_of_recurring_event($dates = [])
	{
		$repeat_type = "";
		$repeat_every = "";
		...
		return ["repeat_type" => $repeat_type, "repeat_every" => $repeat_every];
	}
	...
	{
		$existing_event = $this->ci->Events_model->get_one_where(["google_event_id" => $google_event_id, "created_by" => $user_id]);
	...
	private function _save_calendar_events($data = [], $google_event_id = "")
	{
		...
		$existing_event = $this->ci->Events_model->get_one_where(["google_event_id" => $google_event_id, "created_by" => get_array_value($data, "created_by")]);
