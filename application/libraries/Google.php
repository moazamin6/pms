...
class Google
{
	...
	private function _get_id($name = "", $type = "folder")
	{
		...
		$ids = $stored_ids ? unserialize($stored_ids) : [];
		...
		if(!$this->_is_folder_exists($service, $folder_name))
		{
			...
			else
			{
				...
				if(get_setting("google_drive_parent_folder_id"))
				{
					...
					$fileMetadata = new Google_Service_Drive_DriveFile([
						'name'     => $folder_name,
						'parents'  => [get_setting("google_drive_parent_folder_id")],
						'mimeType' => 'application/vnd.google-apps.folder',
					]);
	...
	public function upload_file($temp_file, $file_name, $folder_name = "", $file_content = "")
	{
		...
		{
			return ["file_name" => $file_name, "file_id" => $file->id, "service_type" => "google"];
		}
	}
	...
	public function move_temp_file($file_name, $new_filename, $folder_name)
	{
		...
		$file = $service->files->update($fileId, $emptyFileMetadata, [
			'addParents'    => $folderId,
		...
		return ["file_name" => $new_filename, "file_id" => $fileId, "service_type" => "google"];
	}
	...
	private function _rename_file($service, $file_id, $new_filename)
	{
		...
		$service->files->update($file_id, $file, [
			'fields' => 'name',
		]);
	}
