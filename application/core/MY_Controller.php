...
class MY_Controller extends CI_Controller
{
	...
	protected function get_access_info($group)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if($module_permission === "own")
					{
						$info->access_type = "own";
					}
					else
					{
						if($module_permission === "read_only")
						{
	...
	protected function access_only_allowed_members()
	{
		...
		{
			if(($this->module_group === "ticket" && ($this->access_type === "specific" || $this->access_type === "assigned_only")) || ($this->module_group === "lead" && $this->access_type === "own") || ($this->module_group === "client" && ($this->access_type === "own" || $this->access_type === "read_only")))
			{
	...
	protected function access_only_allowed_members_or_client_contact($client_id)
	{
		...
		else
		{
			...
			{
				if($this->module_group === "client" && ($this->access_type === "own" || $this->access_type === "read_only"))
				{
	...
	protected function can_create_tasks($in_project = true)
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_create_tasks") == "1")
				{
	...
	protected function _get_currencies_dropdown()
	{
		...
		if($used_currencies)
		{
			...
			$currencies_dropdown = [
				["id" => "", "text" => "- " . lang("currency") . " -"],
				["id" => $default_currency, "text" => $default_currency] // add default currency
			];
	...
	protected function get_hidden_topbar_menus_dropdown()
	{
		...
		{
			$hidden_topbar_menus_dropdown[] = ["id" => $hidden_menu, "text" => lang($hidden_menu)];
		}
	...
	protected function get_clients_and_leads_dropdown($return_json = false)
	{
		...
		$clients = $this->Clients_model->get_all_where(["deleted" => 0], 0, 0, "is_lead")->result();
	...
	protected function get_calendar_filter_dropdown($type = "default")
	{
		...
		$selected_filters_cookie_array = $selected_filters_cookie ? explode('-', $selected_filters_cookie) : ["events"]; //load only events if there is no cookie
		
		$calendar_filter_dropdown = [["id" => "events", "text" => lang("events"), "isChecked" => in_array("events", $selected_filters_cookie_array) ? true : false]];
		...
		if($type !== "lead")
		{
			if($this->login_user->user_type == "staff" && $type == "default")
			{
				...
				{
					$calendar_filter_dropdown[] = ["id" => "leave", "text" => lang("leave"), "isChecked" => in_array("leave", $selected_filters_cookie_array) ? true : false];
				}
				
				//task start dates
				$calendar_filter_dropdown[] = ["id" => "task_start_date", "text" => lang("task_start_date"), "isChecked" => in_array("task_start_date", $selected_filters_cookie_array) ? true : false];
				
				//task deadlines
				$calendar_filter_dropdown[] = ["id" => "task_deadline", "text" => lang("task_deadline"), "isChecked" => in_array("task_deadline", $selected_filters_cookie_array) ? true : false];
			}
			
			//project start dates
			$calendar_filter_dropdown[] = ["id" => "project_start_date", "text" => lang("project_start_date"), "isChecked" => in_array("project_start_date", $selected_filters_cookie_array) ? true : false];
			
			//project deadlines
			$calendar_filter_dropdown[] = ["id" => "project_deadline", "text" => lang("project_deadline"), "isChecked" => in_array("project_deadline", $selected_filters_cookie_array) ? true : false];
		}
	...
	protected function make_labels_dropdown($type = "", $label_ids = "", $is_filter = false, $custom_filter_title = "")
	{
		...
		$labels_dropdown = $is_filter ? [["id" => "", "text" => "- " . ($custom_filter_title ? $custom_filter_title : lang("label")) . " -"]] : [];
	...
	protected function can_edit_projects()
	{
		if($this->login_user->user_type == "staff")
		{
			...
			{
				if(get_array_value($this->login_user->permissions, "can_edit_projects") == "1")
				{
					return true;
				}
			}
	...
	protected function get_user_options_for_query($only_type = "")
	{
		...
		if($this->login_user->user_type == "staff")
		{
			...
			if($only_type !== "client")
			{
				...
				{
					if(get_array_value($this->login_user->permissions, "message_permission") == "specific")
					{
	...
	protected function check_profile_image_dimension($image_file_name = "")
	{
		...
		[$width, $height] = getimagesize($image_file_name);
